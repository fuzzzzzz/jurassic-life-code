...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		//JL
		RecvPropInt( RECVINFO(m_iStress) ),
		RecvPropArray3( RECVINFO_ARRAY(m_nCaseItems), RecvPropEHandle( RECVINFO( m_nCaseItems[0] ) ) ),
		RecvPropInt( RECVINFO(m_iNoteCount) ),
		RecvPropArray3( RECVINFO_ARRAY(m_iNotes), RecvPropInt( RECVINFO( m_iNotes[0] ) ) ),
		RecvPropArray3( RECVINFO_ARRAY(m_bNoteRead), RecvPropBool( RECVINFO( m_bNoteRead[0] ) ) ),		
		RecvPropBool( RECVINFO(m_bVguiUseFull) ),
...
BEGIN_PREDICTION_DATA( C_BasePlayer )
	...
	//JL(TH)
	DEFINE_PRED_ARRAY( m_nCaseItems, FIELD_EHANDLE, MAX_JL_ITEMS, FTYPEDESC_INSENDTABLE ),	
	DEFINE_FIELD( m_iNoteCount, FIELD_INTEGER ),
	DEFINE_PRED_ARRAY( m_iNotes, FIELD_INTEGER, MAX_JL_ITEMS, FTYPEDESC_INSENDTABLE ),	
	DEFINE_PRED_ARRAY( m_bNoteRead, FIELD_BOOLEAN, MAX_JL_ITEMS, FTYPEDESC_INSENDTABLE ),		
...
void C_BasePlayer::Spawn( void )
{
	...
	//JL - red- denis Nedry
	//SetModel( "models/jl/humans/dennis_nedry.mdl" );
	//SetModel( "models/humans/Group03/Female_01.mdl" );
	//SetModel( "models/jl/humans/player/player.mdl" );
...
void C_BasePlayer::DetermineVguiInputMode( CUserCmd *pCmd )
{
	...
	//JL
	if ( GetFlags() & FL_VGUISCREEN_CONTROL )
	{
		//DevMsg("Mouse Delta %d %d\n",pCmd->mousedx,pCmd->mousedy);		
		if (m_pCurrentVguiScreen.Get())
		{
			((C_VGuiScreen*)m_pCurrentVguiScreen.Get())->AddMouseDelta(pCmd->mousedx/sensitivity.GetFloat(),pCmd->mousedy/sensitivity.GetFloat());
		}
	}
	...
	if (GetVguiUseFull() && GetViewModel())
	{
		m_pCurrentVguiScreen = FindNearbyVguiScreen( GetViewModel()->GetAbsOrigin(), GetViewModel()->GetAbsAngles(), GetTeamNumber() );
	}else{
		m_pCurrentVguiScreen = FindNearbyVguiScreen( EyePosition(), EyeAngles(), GetTeamNumber() );
	}
	...
	if (pOldScreen != m_pCurrentVguiScreen)
	{
		...
		if (GetVguiUseFull())
		{
			C_VGuiScreen *old = dynamic_cast<C_VGuiScreen*>(pOldScreen);
			C_VGuiScreen *current = dynamic_cast<C_VGuiScreen*>(m_pCurrentVguiScreen.Get());
			if (old && current)
			{
				int mx,my;
				old->GetMousePos(mx,my);
				current->SetMousePos(mx,my);
			}
		}
	}
...
void C_BasePlayer::AddEntity( void )
{
	...
	//JL (Red) prevent the model in third view to rotate with the angle of the camera.
...
/*static*/ bool C_BasePlayer::ShouldDrawLocalPlayer()
{
	...
	//JL(TH) : always show player models
	//return true;
	
	//JL(red) : The player model is only display in thirdperson view
...
//[JL
EHANDLE C_BasePlayer::GetCurrentVguiScreen()
{
	return m_pCurrentVguiScreen;
}

int C_BasePlayer::GetNoteCount()
{
	return m_iNoteCount; 
}

bool C_BasePlayer::GetVguiUseFull()
{
	return m_bVguiUseFull;
}

const wchar_t* C_BasePlayer::GetNoteTitle(int pos)
{
	if (pos>0 && pos<=m_iNoteCount)
	{
		char Title[32];
		Q_snprintf(Title,sizeof(Title),"#JL_Note_Title_%d",m_iNotes[pos] );	
		DevMsg("title %s\n",Title);
		wchar_t *text = g_pVGuiLocalize->Find(Title);
		if (text)
			return text;
		else
			return L"";
...
const wchar_t* C_BasePlayer::GetNote(int pos)
{
	if (pos>0 && pos<=m_iNoteCount)
	{
		char Title[4096];
		Q_snprintf(Title,sizeof(Title),"#JL_Note_%d",m_iNotes[pos] );	
		DevMsg("text %s %d\n",Title,m_iNotes[pos]);
		wchar_t *text = g_pVGuiLocalize->Find(Title);
		if (text)
			return text;
...
int C_BasePlayer::GetNoteId(int pos)
{
	if (pos>0 && pos<=m_iNoteCount)
	{
		return m_iNotes[pos];
...
bool C_BasePlayer::GetNoteRead(int pos)
{
	if (pos>0 && pos<=m_iNoteCount)
	{
		return m_bNoteRead[pos];
	}
	return false;
}

bool C_BasePlayer::HasNewNote()
{
	for (int i=1;i<=m_iNoteCount;i++)
	{
		if (m_bNoteRead[i]==false)
			return true;
	}
	return false;
}

void C_BasePlayer::SetNoteRead(int pos)
{
	if (pos>0 && pos<=m_iNoteCount)
	{
		//m_bNoteRead[pos]=true;
		char tempcmd[16];
		Q_snprintf(tempcmd,sizeof(tempcmd),"readnote %d",pos);	
		engine->ClientCmd(tempcmd);
	}
}
//JL]