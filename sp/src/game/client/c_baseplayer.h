...
public:
	...
	//JL - Should this object cast shadows?
	...
	//JL(TH) : 14/02/09
	//void SetCurrentVguiScreen(C_BaseEntity* current);
	EHANDLE GetCurrentVguiScreen();
...
protected:
	...
	//JL:red thirdveiw on player death
...
protected:
	...
	int m_iStress;
	int m_iNoteCount;
	int m_iNotes[MAX_JL_NOTES];
	bool m_bNoteRead[MAX_JL_NOTES];
	bool m_bVguiUseFull;
...
public:
	...
	//JL(TH)
	//C_BaseCombatWeapon* GetItem(int i) { return m_nItems[i];};
	//string_t 
	C_BaseCombatWeapon* GetCaseItem(int i) { return m_nCaseItems[i]; };
	//void SetCaseItem(int i, C_BaseCombatWeapon* weapon) { m_nCaseItems[i].Set(weapon); };
	//void SetItem(int i,int value) { m_nItems[i] = value;};
	int GetStress() { return m_iStress; };
	//void SetStress(int i) { m_iStress = i; };

	int				GetNoteCount();
	const wchar_t*	GetNoteTitle(int pos);
	const wchar_t*	GetNote(int pos);
	int				GetNoteId(int pos);
	bool			GetNoteRead(int pos);
	bool			GetVguiUseFull();
	bool			HasNewNote();
	void			SetNoteRead(int pos);
