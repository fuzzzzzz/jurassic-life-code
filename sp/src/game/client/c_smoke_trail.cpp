...
MosquitoEffect::MosquitoEffect( const char *pDebugName ) : CSimpleEmitter( pDebugName )
{
}


MosquitoEffect* MosquitoEffect::Create( const char *pDebugName )
{
	return new MosquitoEffect( pDebugName );
}
...
// Output : Vector
//-----------------------------------------------------------------------------
void MosquitoEffect::UpdateVelocity( SimpleParticle *pParticle, float timeDelta )
{
	float	speed = VectorNormalize( pParticle->m_vecVelocity );
	Vector	offset;
...
//			timeDelta - 
//-----------------------------------------------------------------------------
Vector MosquitoEffect::UpdateColor( const SimpleParticle *pParticle )
{
	Vector	color;
	float	ramp = ((float)pParticle->m_uchStartAlpha/255.0f) * sin( M_PI * (pParticle->m_flLifetime / pParticle->m_flDieTime) );//1.0f - ( pParticle->m_flLifetime / pParticle->m_flDieTime );
	...
	//color[0] = ( (float) pParticle->m_uchColor[0] ) / 255.0f;
	//color[1] = ( (float) pParticle->m_uchColor[1] ) / 255.0f;
	//color[2] = ( (float) pParticle->m_uchColor[2] ) / 255.0f;
...
// Output : float
//-----------------------------------------------------------------------------
float MosquitoEffect::UpdateAlpha( const SimpleParticle *pParticle )
{
	//return 200;
...
EXPOSE_PROTOTYPE_EFFECT( MosquitoParticle, C_MosquitoParticle );

IMPLEMENT_CLIENTCLASS_DT( C_MosquitoParticle, DT_MosquitoParticle, MosquitoParticle )
	RecvPropFloat(RECVINFO(m_flSpawnRate)),
	RecvPropFloat(RECVINFO(m_flParticleLifetime)),
	RecvPropFloat(RECVINFO(m_flStartSize)),
	RecvPropFloat(RECVINFO(m_flEndSize)),
...
C_MosquitoParticle::C_MosquitoParticle( void )
{
	m_pParticleMgr			= NULL;
...
C_MosquitoParticle::~C_MosquitoParticle()
{
	if ( m_pParticleMgr != NULL )
	{
		m_pParticleMgr->RemoveEffect( &m_ParticleEffect );
...
// Input  : bnewentity - 
//-----------------------------------------------------------------------------
void C_MosquitoParticle::OnDataChanged( DataUpdateType_t updateType )
{
	DevMsg("Mosquito data changed\n");
	C_BaseEntity::OnDataChanged( updateType );
...
//			*pArgs - 
//-----------------------------------------------------------------------------
void C_MosquitoParticle::Start( CParticleMgr *pParticleMgr, IPrototypeArgAccess *pArgs )
{
	//Add us into the effect manager
	if( pParticleMgr->AddEffect( &m_ParticleEffect, this ) == false )
		return;
	...
	//Create our main effect
	m_pMosquitoEffect = MosquitoEffect::Create( "C_MosquitoParticle" );
	
	if ( m_pMosquitoEffect == NULL )
		return;

	m_hMaterial	= m_pMosquitoEffect->GetPMaterial( "particle/fly" );

	m_pMosquitoEffect->SetSortOrigin( GetAbsOrigin() );
	m_pMosquitoEffect->SetNearClip( 64, 128 );

	//C_BaseEntity::EmitAmbientSound(0,GetAbsOrigin(),"mosquito");
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_MosquitoParticle::AddParticles( void )
{
	//Spores
	Vector	offset;
	Vector	dir;
	...
	for ( int i = 0; i < 4; i++ )
	{
		//Add small particle to the effect's origin
		offset.Random( -m_flSpawnRadius, m_flSpawnRadius );
		sParticle = (SimpleParticle *) m_pMosquitoEffect->AddParticle( sizeof(SimpleParticle), m_hMaterial, GetAbsOrigin()+offset );
		...
		sParticle->m_uchStartAlpha	= 225;
		sParticle->m_uchEndAlpha	= 196;
		sParticle->m_uchStartSize	= Helper_RandomInt( 1, 2 );
		sParticle->m_uchEndSize		= 1;
	...
	//Add smokey bits
	offset.Random( -(m_flSpawnRadius * 0.5), (m_flSpawnRadius * 0.5) );
	sParticle = (SimpleParticle *) m_pMosquitoEffect->AddParticle( sizeof(SimpleParticle), g_Mat_DustPuff[1], GetAbsOrigin()+offset );
	...
	sParticle->m_uchColor[0]	= 16;
	sParticle->m_uchColor[1]	= 16;
	sParticle->m_uchColor[2]	= 16;
...
ConVar cl_mosquitoclipdistance( "cl_mosquitoclipdistance", "512", FCVAR_CHEAT | FCVAR_CLIENTDLL );
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : fTimeDelta - 
//-----------------------------------------------------------------------------
void C_MosquitoParticle::Update( float fTimeDelta )
{
	if( m_bEmit )
	{
		float tempDelta = fTimeDelta;
		...
		//Lower the spawnrate by half if we're far away from it.
		if ( cl_mosquitoclipdistance.GetFloat() <= flDist )
		{
			if ( m_flSpawnRate == m_flPreviousSpawnRate )
			{
				m_flPreviousSpawnRate = m_flSpawnRate * 0.5f;
...
void C_MosquitoParticle::SimulateParticles( CParticleSimulateIterator *pIterator )
{
	StandardParticle_t *pParticle = (StandardParticle_t*)pIterator->GetFirst();
	while ( pParticle )
	{
...
void C_MosquitoParticle::RenderParticles( CParticleRenderIterator *pIterator )
{
}
