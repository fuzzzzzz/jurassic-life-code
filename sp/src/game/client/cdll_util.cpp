...
//JL
int strReplace(char *Text,const char *Find,const char *Replace)
{
	int Lenght = strlen(Text);
	int LenghtFind = strlen(Find);
	int LenghtReplace = strlen(Replace);
	int NDResult=0;
	for(int i=0;i<Lenght-LenghtFind+1;i++)
	{
		for(int j=0;j<LenghtFind;j++)
		if(Text[i+j]!=Find[j]) goto Nop;
		NDResult++;

		if(LenghtFind==LenghtReplace)
		{
			memcpy(&Text[i],Replace,LenghtReplace);
		}
		else if(LenghtFind<LenghtReplace)
		{
			for(int k=Lenght-1;k>=i;k--)
			Text[k + (LenghtReplace-LenghtFind)] = Text[k];
			memcpy(&Text[i],Replace,LenghtReplace);
		}
		else if(LenghtFind>LenghtReplace)
		{
			for(int k=i+LenghtFind;k<Lenght;k++)
			Text[k + (LenghtReplace-LenghtFind)] = Text[k];
			memcpy(&Text[i],Replace,LenghtReplace);
		}
		Text[Lenght + (LenghtReplace - LenghtFind)] = '\0';
		Lenght = strlen(Text);
		LenghtFind = strlen(Find);
		LenghtReplace = strlen(Replace);
		Nop:;
	}
	return NDResult;
}
...
//By CameleonTH
int strfind(char* text,char* tofind)
{
     int i,j,n=0;
     bool quit=true;
     for (i=0;i<strlen(text);i++)
     {
         ...
         if (text[i]==tofind[0])
         {       
            for (j=1;j<strlen(tofind);j++)
            {
                if (text[i+j]!=tofind[j])                   
                   quit=false;                   
            }
            if (quit==true)
               n++;
