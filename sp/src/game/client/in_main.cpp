...
//JL(TH)
void IN_Inventory(const CCommand &args) 
{  
   KeyDown( &in_inventory, args[1] ); 
	engine->ClientCmd( "ToggleInventory"); 
}

void IN_DropUp(const CCommand &args) { KeyDown( &in_drop, args[1] ); }
void IN_DropDown(const CCommand &args) { KeyUp( &in_drop, args[1] ); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	//JL(TH)
	CalcButtonBits( bits, IN_INVENTORY, s_ClearInputState, &in_inventory, bResetState );
...
//JL(TH)
static ConCommand showinventory( "inventory", IN_Inventory );
//static ConCommand hideinventory( "-inventory", IN_Inventory );
static ConCommand startdrop( "+drop", IN_DropUp );
static ConCommand enddrop( "-drop", IN_DropDown );
