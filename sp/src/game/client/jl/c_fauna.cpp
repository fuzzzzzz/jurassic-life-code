...
ConVar FaunaDebug( "fauna_debug", "0", FCVAR_CHEAT, "Show debug info for fauna creatures" );
 ...
 */
class C_Fauna : public C_BaseAnimating
{
public:
	DECLARE_CLASS( C_Fauna, C_BaseAnimating );
	DECLARE_CLIENTCLASS();
...
private:
	friend void RecvProxy_FaunaOriginX( const CRecvProxyData *pData, void *pStruct, void *pOut );
	friend void RecvProxy_FaunaOriginY( const CRecvProxyData *pData, void *pStruct, void *pOut );
	friend void RecvProxy_FaunaOriginZ( const CRecvProxyData *pData, void *pStruct, void *pOut );
...
//-----------------------------------------------------------------------------
void RecvProxy_FaunaOriginX( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	C_Fauna *fauna = (C_Fauna *)pStruct;
	float *out = (float *)pOut;

	*out = pData->m_Value.m_Float + fauna->m_poolOrigin.x;
}

void RecvProxy_FaunaOriginY( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	C_Fauna *fauna = (C_Fauna *)pStruct;
	float *out = (float *)pOut;

	*out = pData->m_Value.m_Float + fauna->m_poolOrigin.y;
}

void RecvProxy_FaunaOriginZ( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	C_Fauna *fauna = (C_Fauna *)pStruct;
	float *out = (float *)pOut;

	*out = pData->m_Value.m_Float + fauna->m_poolOrigin.z;
}

IMPLEMENT_CLIENTCLASS_DT_NOBASE( C_Fauna, DT_CFlockFauna, CFlockFauna )

	RecvPropVector( RECVINFO(m_poolOrigin) ),

	RecvPropFloat( RECVINFO_NAME( m_actualPos.x, m_x ), 0, RecvProxy_FaunaOriginX ),
	RecvPropFloat( RECVINFO_NAME( m_actualPos.y, m_y ), 0, RecvProxy_FaunaOriginY ),
	//RecvPropFloat( RECVINFO_NAME( m_actualPos.z, m_z ) ),
	RecvPropFloat( RECVINFO_NAME( m_actualPos.z, m_z ), 0, RecvProxy_FaunaOriginZ ),
...
//-----------------------------------------------------------------------------
void C_Fauna::ClientThink()
{
	if (FaunaDebug.GetBool())
	{
	...
	// keep creature motion smooth by correcting towards actual server position
...
//-----------------------------------------------------------------------------
void C_Fauna::OnDataChanged( DataUpdateType_t type )
{
	//if (!m_gotUpdate)
