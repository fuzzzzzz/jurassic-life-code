...
CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectTazer)
CLIENTEFFECT_MATERIAL( "effects/stunstick" )
CLIENTEFFECT_REGISTER_END()

class C_WeaponTazer : public C_BaseHLBludgeonWeapon
{
	DECLARE_CLASS( C_WeaponTazer, C_BaseHLBludgeonWeapon );
public:
	DECLARE_CLIENTCLASS();
	DECLARE_PREDICTABLE();
...
//			*pOut - 
//-----------------------------------------------------------------------------
void RecvProxy_TazerActive( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	bool state = *((bool *)&pData->m_Value.m_Int);

	C_WeaponTazer *pWeapon = (C_WeaponTazer *) pStruct;

	if ( state )
	{
		// Turn on the effect
		pWeapon->StartTazerEffect();
	}
	else
	{
		// Turn off the effect
		pWeapon->StopTazerEffect();
	}

	*(bool *)pOut = state;
}

STUB_WEAPON_CLASS_IMPLEMENT( weapon_tazer, C_WeaponTazer );

IMPLEMENT_CLIENTCLASS_DT( C_WeaponTazer, DT_WeaponTazer, CWeaponTazer )
	RecvPropInt( RECVINFO(m_bActive), 0, RecvProxy_TazerActive ),
END_RECV_TABLE()