...
//JL
void BitmapImage::DoPaint( vgui::VPANEL panel, float x0, float y0, float x1, float y1)
{
	vgui::surface()->DrawSetTexture( m_nTextureId );
	...
	if ( panel )
	{
		vgui::ipanel()->GetSize(panel, wide, tall );
	}
	else
	{
		wide = m_Size[0];
	...
	vgui::surface()->DrawTexturedSubRect( m_pos[0], m_pos[1], wide, tall,
		x0, y0, x1, y1);
}
