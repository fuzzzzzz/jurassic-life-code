...
// Draws the screen effect
//-----------------------------------------------------------------------------
inline void DrawScreenEffectMaterialInTexture( IMaterial *pMaterial,ITexture *pTex, int x, int y, int w, int h )
{
	if (!pTex)
	{
		pTex = GetFullFrameFrameBufferTexture( 0 );
	}
	Rect_t actualRect;
	UpdateScreenEffectTexture( 0, x, y, w, h, false, &actualRect );
	...
	pRenderContext->DrawScreenSpaceRectangle( pMaterial, x, y, w, h,
		actualRect.x, actualRect.y, actualRect.x+actualRect.width-1, actualRect.y+actualRect.height-1, 
		pTex->GetActualWidth(), pTex->GetActualHeight() );
