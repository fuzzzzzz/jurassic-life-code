...
//[JL
//-----------------------------------------------------------------------------
// JL(TH) : Radial blur function
// To toggle the blur on and off
ConVar jl_radialblur("jl_radialblur", "0", 0, "Radial Blur"); 
// The amount of alpha to use when adding the FB to our custom buffer
ConVar jl_radialblur_alpha("jl_radialblur_alpha", "0.1", 0, "Radial Blur Alpha");
// Number of sample to create the radial blur
ConVar jl_radialblur_sample("jl_radialblur_sample", "10", 0, "Radial Blur Samples number"); 
// Intensity of radial blur
ConVar jl_radialblur_power("jl_radialblur_power", "1", 0, "Radial Blur intensity"); 

//JL(TH) : Depth of Field CVar
ConVar jl_post_dof("jl_post_dof", "0", 0, "JL Depth Of Field"); 
// Blur CVar
ConVar jl_post_blur("jl_post_blur", "0", 0, "Blur"); 
ConVar jl_post_blur_pass( "jl_post_blur_pass", "5", FCVAR_SPONLY );
static ConVar jl_post_blur_power( "jl_post_blur_power", "0.003", FCVAR_CHEAT );

//Special effect
ConVar jl_post_blur_fade_enable( "jl_post_blur_fade_enable", "0", FCVAR_SPONLY );
ConVar jl_post_blur_fade_time( "jl_post_blur_fade_time", "0.25", FCVAR_SPONLY );
ConVar jl_post_blur_fade_power( "jl_post_blur_fade_power", "0.003", FCVAR_SPONLY );
double m_LastTime;

ConVar jl_post_grain( "jl_post_grain", "1", FCVAR_ARCHIVE);
ConVar jl_post_grain_alpha( "jl_post_grain_alpha", "0.13", FCVAR_ARCHIVE,"Grain effect alpha",true,0.0,true,1.0);
//JL]
...
CLIENTEFFECT_REGISTER_BEGIN( PrecachePostProcessingEffects )
	...
	CLIENTEFFECT_MATERIAL( "shaders/post_dof" )
	CLIENTEFFECT_MATERIAL( "shaders/post_blur" )
	CLIENTEFFECT_MATERIAL( "shaders/post_grain" )
	CLIENTEFFECT_MATERIAL( "shaders/post_radial" )
...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	//JL(TH) Draw Depth of Field effect
	PerformPostProcessEffects( view.x, view.y, view.width, view.height ); 
	//JL(TH) Draw the blur effect
	DoRadialBlur(view.width, view.height);
	//JL(TH) Draw grain effect
	DoGrainEffect(view.width, view.height);

//these three lines breaks the hdr support and fucked all vgui panels --red	
//	pRenderContext.GetFrom( materials );
//			pRenderContext->SetToneMappingScaleLinear(Vector(0,0,0));
//			pRenderContext.SafeRelease();
...
//JL(TH) : TO FIX
void CViewRender::DoRadialBlur( int width, int height )
{
	if ( !jl_radialblur.GetBool() ) 
		return;

	bool found;
	int sample = jl_radialblur_sample.GetInt();
	float power = jl_radialblur_power.GetFloat();
	IMaterial *pMatScreen = materials->FindMaterial( "shaders/post_radial", TEXTURE_GROUP_CLIENT_EFFECTS, true );
	if (pMatScreen)
	{
		IMaterialVar* mv = pMatScreen->FindVar( "$alpha", &found, false );
		if (found)
		{
			mv->SetFloatValue( jl_radialblur_alpha.GetFloat() );
		}
		CMatRenderContextPtr pRenderContext( materials );
		for (int i=0;i<=sample;i++)
		{
			DrawScreenEffectMaterial(pMatScreen,i*power*ScreenWidth()/100.0, i*power*ScreenHeight()/100.0, ScreenWidth()-i*power*ScreenWidth()/100.0, ScreenHeight()-i*power*ScreenHeight()/100.0/*, ScreenWidth(), ScreenHeight()*/);
		}
		DrawScreenEffectMaterial(pMatScreen,20,20,width-40,height-40);
	}
}

//JL(TH) : postProcess Function
void CViewRender::PerformPostProcessEffects( int x, int y, int width, int height ) 
{ 
	if (jl_post_blur_fade_enable.GetBool())
	{
		if (jl_post_blur.GetBool()==false)
		{
			m_LastTime = Plat_FloatTime();
			jl_post_blur.SetValue(1);
			jl_post_blur_power.SetValue(0.0f);
		}
		if (jl_post_blur_power.GetFloat()<jl_post_blur_fade_power.GetFloat())
		{
			float time = Plat_FloatTime()-m_LastTime;
			float val = time / jl_post_blur_fade_time.GetFloat();
			m_LastTime = Plat_FloatTime();
			jl_post_blur_power.SetValue(jl_post_blur_power.GetFloat()+val*jl_post_blur_fade_power.GetFloat());
		}else{
			m_LastTime = Plat_FloatTime();
		}
	}else{
		if (jl_post_blur.GetBool())
		{
			if (jl_post_blur_power.GetFloat()>0)
			{
				float time = Plat_FloatTime()-m_LastTime;
				float val = time / jl_post_blur_fade_time.GetFloat();
				m_LastTime = Plat_FloatTime();
				...
				jl_post_blur.SetValue(0);
				jl_post_blur_power.SetValue(0);
			}
		}
	}

	if ( jl_post_dof.GetInt()==0 && jl_post_blur.GetInt()==0) //no active effects, bail out 
		return; 

	IMaterial *pBlurMat = materials->FindMaterial( "shaders/post_blur", TEXTURE_GROUP_CLIENT_EFFECTS, true ); 
	IMaterial *pDofMat = materials->FindMaterial( "shaders/post_dof", TEXTURE_GROUP_CLIENT_EFFECTS, true ); 

	ITexture *pTarget = GetFullFrameFrameBufferTexture(1);

	if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80 ) 
	{
		if ( jl_post_dof.GetInt() == 1 && pBlurMat && pDofMat ) 
		{ 
			//Render blur pass	
			CMatRenderContextPtr pRenderContext( materials );

			//pRenderContext->SetRenderTarget(pTex);
			pRenderContext->CopyRenderTargetToTextureEx(pTarget,0,NULL,NULL);
			//pRenderContext->CopyRenderTargetToTextureEx( pTexture, 0, &srcRect, bDestFullScreen ? NULL : &destRect );
			for (int i=0;i<jl_post_blur_pass.GetInt();i++)
			{
				//DrawScreenEffectMaterialInTexture(pDofMat, pTarget, x, y, width, height ); 
				DrawScreenEffectMaterial( pBlurMat, x, y, width, height ); 
			}
			//Render dof pass
			DrawScreenEffectMaterial( pDofMat, x, y, width, height ); 
		}
		
		if ( jl_post_blur.GetInt() == 1 && pBlurMat ) 
		{ 
			for (int i=0;i<jl_post_blur_pass.GetInt();i++)
			{
				DrawScreenEffectMaterial( pBlurMat, x, y, width, height ); 
...
//
void CViewRender::DoGrainEffect(int width, int height)
{
	const int GrainSize = 128;

	if (jl_post_grain.GetBool())
	{
		IMaterial *pGrainMat = materials->FindMaterial( "shaders/post_grain", TEXTURE_GROUP_CLIENT_EFFECTS, true ); 
		if (pGrainMat)
		{
			bool found;
			IMaterialVar* mv = pGrainMat->FindVar( "$alpha", &found, false );
			if (found)
			{
				mv->SetFloatValue( jl_post_grain_alpha.GetFloat() );
			}
			CMatRenderContextPtr pRenderContext( materials );
			for (int x=0;x<width;x+=GrainSize)
			{
				...
				{
					pRenderContext->DrawScreenSpaceRectangle( pGrainMat, 
						x, y, GrainSize, GrainSize,
						0,0,GrainSize,GrainSize,GrainSize,GrainSize );
				}
			}
		}
	}
