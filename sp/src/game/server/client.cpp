...
//JL(TH) : Flashlight swap
//-----------------------------------------------------------------------------
// Purpose: Quickly switch to the flashlight, or back to previous item
//-----------------------------------------------------------------------------
void CC_Player_FlashLightSwap( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( UTIL_GetCommandClient() );
	...
	if ( pPlayer )
	{
		...
		if ( pWeapon )
		{
			...
			if ( !Q_stricmp( strWeaponName, "weapon_flashlight" ) )
			{
				pPlayer->SelectLastItem();
			}
			else
			{
				pPlayer->SelectItem( "weapon_flashlight" );
...
static ConCommand falshlightswap("flashlight_swap", CC_Player_FlashLightSwap, "Automatically swaps the current weapon for the flashlight and back again." );
