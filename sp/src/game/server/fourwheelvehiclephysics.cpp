...
//	DEFINE_PHYSPTR_ARRAY( m_pWheels ),
	...
	//JL
	DEFINE_FIELD( m_bElevator, FIELD_BOOLEAN ),
...
void CFourWheelVehiclePhysics::DisableMotion( void )
{
	...
	if (m_pElevator)
	{
		m_pElevator->SetVelocity( &vecZero, &angNone );
		m_pElevator->EnableMotion( false );
	}
}
...
int CFourWheelVehiclePhysics::VPhysicsGetObjectList( IPhysicsObject **pList, int listMax )
{
	...
	//JL(TH) : Add elevator
	count = AddPhysToList( pList, listMax, count, m_pElevator );
...
//JL(TH) : Elevator
void CFourWheelVehiclePhysics::SetElevatorEnable(bool enable, IPhysicsObject *physobj, IPhysicsObject *physobjbase)
{
	m_bElevator = enable;
	if (enable && physobj!=NULL)
	{
		m_pElevator = physobj;

		if (m_pElevator)
		{
			//m_pElevator->SetMass(2);
			m_bElevatorUpdate=true;
			UpdateElevator();

			matrix3x4_t ref,attached;
			m_pElevatorConstraint->GetConstraintTransform(&ref,&attached);
			/*
			DevMsg("Attached\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n",	attached.m_flMatVal[0][0],attached.m_flMatVal[0][1],attached.m_flMatVal[0][2],attached.m_flMatVal[0][3],
						attached.m_flMatVal[1][0],attached.m_flMatVal[1][1],attached.m_flMatVal[1][2],attached.m_flMatVal[1][3],
						attached.m_flMatVal[2][0],attached.m_flMatVal[2][1],attached.m_flMatVal[2][2],attached.m_flMatVal[2][3]);
			DevMsg("Ref\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n",	ref.m_flMatVal[0][0],ref.m_flMatVal[0][1],ref.m_flMatVal[0][2],ref.m_flMatVal[0][3],
						ref.m_flMatVal[1][0],ref.m_flMatVal[1][1],ref.m_flMatVal[1][2],ref.m_flMatVal[1][3],
						ref.m_flMatVal[2][0],ref.m_flMatVal[2][1],ref.m_flMatVal[2][2],ref.m_flMatVal[2][3]);
			*/
		}
	}
	if (!enable)
	{
		physenv->DestroyConstraint(m_pElevatorConstraint);
		m_pElevatorConstraint = NULL;
	}
}

bool CFourWheelVehiclePhysics::GetElevatorEnable()
{
	return m_bElevator;
}

void CFourWheelVehiclePhysics::UpdateElevator()
{
	if (m_pElevator && m_bElevatorUpdate)
	{
		if (m_pElevatorConstraint)
			m_pElevatorConstraint->Deactivate();

		Vector pos, forward, right, up;
		QAngle angle;
		GetAttachment( "vehicle_elevator", pos, angle );
		AngleVectors(angle,&forward,&right,&up);
		m_pElevator->SetPosition(pos+forward*2,angle,false);

		constraint_fixedparams_t fixed;
		fixed.Defaults();
		fixed.InitWithCurrentObjectState( m_pOuter->VPhysicsGetObject(), m_pElevator );
		
		/*IPhysicsConstraint *tempconstraint;
		if (m_pElevatorConstraint)
		{
			delete m_pElevatorConstraint;
			//m_pElevatorConstraint=NULL;
		}
		tempconstraint = physenv->CreateFixedConstraint(m_pOuter->VPhysicsGetObject(),m_pElevator,NULL,fixed);
		
		m_pElevatorConstraint=tempconstraint;*/

		if (m_pElevatorConstraint==NULL)
		{
			m_pElevatorConstraint = physenv->CreateFixedConstraint(m_pOuter->VPhysicsGetObject(),m_pElevator,NULL,fixed);
			//m_pElevatorConstraintBase = physenv->CreateFixedConstraint(m_pOuter->VPhysicsGetObject(),m_pElevatorBase,NULL,fixed);
		}else{
			//matrix3x4_t temp;
			//temp.Init(Vector(1,0,0),Vector(0,1,0),Vector(0,0,1),Vector(0,0,0));
			
			//m_pElevatorConstraint->UpdateRagdollTransforms(temp,fixed.attachedRefXform);
			//m_pElevatorConstraint->Activate();
			//m_pElevatorConstraint->UpdateRagdollTransforms(fixed.attachedRefXform,fixed.attachedRefXform);			
			
			//physenv->Simulate(0);
			/*
			DevMsg("fixed.attachedRefXform upd\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n\
%f\t %f\t %f\t %f\n",	fixed.attachedRefXform.m_flMatVal[0][0],fixed.attachedRefXform.m_flMatVal[0][1],fixed.attachedRefXform.m_flMatVal[0][2],fixed.attachedRefXform.m_flMatVal[0][3],
						fixed.attachedRefXform.m_flMatVal[1][0],fixed.attachedRefXform.m_flMatVal[1][1],fixed.attachedRefXform.m_flMatVal[1][2],fixed.attachedRefXform.m_flMatVal[1][3],
						fixed.attachedRefXform.m_flMatVal[2][0],fixed.attachedRefXform.m_flMatVal[2][1],fixed.attachedRefXform.m_flMatVal[2][2],fixed.attachedRefXform.m_flMatVal[2][3]);
			*/
			physenv->DestroyConstraint(m_pElevatorConstraint);
			m_pElevatorConstraint = physenv->CreateFixedConstraint(m_pOuter->VPhysicsGetObject(),m_pElevator,NULL,fixed);
			//m_pElevator->SetPosition(pos+forward,angle,false);
...
%f\t %f\t %f\t %f\n",	ref.m_flMatVal[0][0],ref.m_flMatVal[0][1],ref.m_flMatVal[0][2],ref.m_flMatVal[0][3],
		...
		m_bElevatorUpdate=false;
	}
}

void CFourWheelVehiclePhysics::CallUpdateElevator()
{
	m_bElevatorUpdate=true;
}
