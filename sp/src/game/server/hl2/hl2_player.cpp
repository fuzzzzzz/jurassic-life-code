...
{
	//JL - (disable in 2013 SDK) Here we create and init the player animation state.
...
void CHL2_Player::Precache( void )
{
	...
	//JL
	PrecacheModel( "models/jl/humans/player/player.mdl" );
	PrecacheModel( "models/humans/Group03/Female_01.mdl" );
	PrecacheModel( "models/jl/humans/dennis_nedry/dennis_nedry.mdl" );
}
...
//-----------------------------------------------------------------------------
// JL (red) 
// Purpose: player animations 
//-----------------------------------------------------------------------------
void CHL2_Player::SetAnimation( PLAYER_ANIM playerAnim )
{	
	//doesnt work when i return to the cbaseplayer class in player.cpp -red
	//return BaseClass::SetAnimation();
	int animDesired;
	char szAnim[64];
	...
	else if (playerAnim == PLAYER_ATTACK1)
	{
		...
		{
			idealActivity = ACT_RANGE_ATTACK1; //ACT_HL2MP_GESTURE_RANGE_ATTACK;
		}
	}
	...
	else if ( playerAnim == PLAYER_IDLE || playerAnim == PLAYER_WALK )
	{
		...
		*/
		...
		{
			if ( speed == 0 )
				idealActivity = ACT_HL2MP_IDLE; //ACT_HOVER;
			else
				idealActivity = ACT_IDLE; //ACT_SWIM;
		}
		/*
...
void CHL2_Player::HandleSpeedChanges( void )
{
	...
	bool bWantSprint = ( bCanSprint && (m_nButtons & IN_SPEED) && IsSuitEquipped() && (m_nButtons & IN_SPEED) );
...
void CHL2_Player::Spawn(void)
{
	...
	//JL(red) : change player models
	SetModel( "models/jl/humans/player/player.mdl" );
	//SetModel( "models/jl/humans/dennis_nedry/dennis_nedry.mdl" );
#endif
#endif
...
void CHL2_Player::StartSprinting( void )
{
	...
	//JL(TH) : Holster weapons when player sprint
	if (GetActiveWeapon())
...
void CHL2_Player::StopSprinting( void )
{
	...
	//JL(TH) : Deploy weapon when player stop sprint
	if (GetActiveWeapon())
...
bool CHL2_Player::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	//JL(TH) : Can't change weapon when player is sprinting
	if (IsSprinting())
		return false;
