...
// ========================================================================
//	>> CItem_MedusaLoader // jurassic life
// ========================================================================
class CItem_MedusaLoader : public CItem
{
public:
	DECLARE_CLASS( CItem_MedusaLoader, CItem );

	void Precache( void )
	{
		PrecacheModel ("models/jl/items/medusa_loader.mdl");
	}
	void Spawn( void )
	{ 
		Precache( );
		SetModel( "models/jl/items/medusa_loader.mdl");
		BaseClass::Spawn( );
	}

	bool MyTouch( CBasePlayer *pPlayer )
	{
		if (ITEM_GiveAmmo( pPlayer, SIZE_AMMO_MEDUSA, "Medusa"))
		{
			if ( g_pGameRules->ItemShouldRespawn( this ) == GR_ITEM_RESPAWN_NO )
			{
				UTIL_Remove(this);	
...
LINK_ENTITY_TO_CLASS(item_medusa_loader, CItem_MedusaLoader);
