...
//JL
#define BIRDTYPE_TOUCAN 4
#define BIRDTYPE_QUETZAL 5
//
// Spawnflags.
//
...
// Purpose: Seagull. Crow with a different model.
//-----------------------------------------------------------------------------
class CNPC_Bird : public CNPC_Crow
{
	DECLARE_CLASS( CNPC_Bird, CNPC_Crow );

public:
	
	void Spawn( void )
	{
		SetModelName( AllocPooledString("models/jl/animals/toucan.mdl") );
		BaseClass::Spawn();

		m_iBirdType = BIRDTYPE_TOUCAN;
	}
...
// Purpose: Seagull. Crow with a different model.
//-----------------------------------------------------------------------------
class CNPC_Quetzal : public CNPC_Crow
{
	DECLARE_CLASS( CNPC_Quetzal, CNPC_Crow );

public:
	
	void Spawn( void )
	{
		SetModelName( AllocPooledString("models/jl/animals/quetzal.mdl") );
		BaseClass::Spawn();

		m_iBirdType = BIRDTYPE_QUETZAL;
	}

	void PainSound( const CTakeDamageInfo &info )
	{
		EmitSound( "NPC_Quetzal.Pain" );
	}
