...
void CPropJeep::Precache( void )
{
	...
	//JL(TH) : Precache Horn sound
	PrecacheScriptSound( "PropJeep.Horn" ); // PropJee.Klaxon
...
{
	//JL(TH) : Play Horn sound
	if ( iButtonsDown & IN_RELOAD )
	{
		CPASAttenuationFilter sndFilter( this, "PropJeep.Horn" );
		//GetSoundDuration( "PropJeep.Klaxon",NULL);
		EmitSound( sndFilter, entindex(), "PropJeep.Horn" );
	}

	int iButtons = ucmd->buttons;
	
	//JL(TH) : ACtive headlights
