...
extern ConVar sk_plr_num_spas12_pellets;
...
private:
	...
	float	fl_NextAnimTime;	// next animation time after unfolding stock
...
// Readiness activities (not aiming)
	{ ACT_IDLE_RELAXED,				ACT_IDLE_SPAS12_RELAXED,		false },//never aims
	{ ACT_IDLE_STIMULATED,			ACT_IDLE_SPAS12_STIMULATED,	false },
	{ ACT_IDLE_AGITATED,			ACT_IDLE_SPAS12_AGITATED,		false },//always aims
...
bool CWeaponSpas12::StartReload( void )
{
	...
	SendWeaponAnim( ACT_SPAS12_RELOAD_START );
...
void CWeaponSpas12::FinishReload( void )
{
	...
	SendWeaponAnim( ACT_SPAS12_RELOAD_FINISH );
...
void CWeaponSpas12::Pump( void )
{
	...
	// Finish reload animation
	SendWeaponAnim( ACT_SPAS12_PUMP );
...
void CWeaponSpas12::PrimaryAttack( void )
{
	...
	if( gpGlobals->curtime < fl_NextAnimTime ){
		return;
	}
	...
	/*
	if ( ( ( pPlayer-> m_nButtons & ~IN_ATTACK ) && ( pPlayer-> m_nButtons & !IN_ATTACK2) && m_bInZoom ) )
	{
			SendWeaponAnim( ACT_SPAS12_SD_PRIMARYATTACK );
	}
	else
	{
	...
	*/
	...
	pPlayer->FireBullets( sk_plr_num_spas12_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, true, true );
...
void CWeaponSpas12::SecondaryAttack( void )
{
	...
	if ( pPlayer->m_afButtonPressed ~& IN_ATTACK2 && IN_ZOOM ) //m_bInZoom == true)
	{
		SendWeaponAnim( ACT_VM_SECONDARYATTACK );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSpas12::CheckZoomToggle( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );

	if ( pPlayer->m_afButtonPressed & IN_ATTACK2 && !m_bButtonPressed ) 
	{
			ToggleZoom();
			m_bButtonPressed = true;
			if( m_bInZoom ){
				SendWeaponAnim( ACT_SPAS12_STOCK_DOWN );
				fl_NextAnimTime = SequenceDuration() + gpGlobals->curtime;
			}
			else{
				SendWeaponAnim( ACT_SPAS12_STOCK_UP );
			}
	}
	else{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSpas12::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	...
	{
		if ( pPlayer->SetFOV( this, 50, 0.1f ) ) //dezoom = 88
		{
			m_bInZoom = true;
			//SendWeaponAnim( ACT_SPAS12_STOCK_IDLE );
		}
	}
}
...
//-----------------------------------------------------------------------------
void CWeaponSpas12::StopEffects( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSpas12::ItemBusyFrame( void )
{
...
void CWeaponSpas12::ItemPostFrame( void )
{
	...
	if ((m_bDelayedFire2 || pOwner->m_nButtons & IN_ATTACK2)&&(m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		else
		{
			...
			SecondaryAttack(); // hit with the cross ? -red
		}
	}
...
void CWeaponSpas12::WeaponIdle( void )
{
	...
	if( fl_NextAnimTime > gpGlobals->curtime ){
		return;
	}

	//If we're on a target, play the new anim
	if ( pOwner->IsOnTarget() )
	{
		//SendWeaponAnim( ACT_VM_IDLE_ACTIVE );
		SendWeaponAnim( ACT_SPAS12_IDLE );

	}
	if ( m_bInZoom && !m_bInReload)
	{
		SendWeaponAnim( ACT_SPAS12_STOCK_UP );
	}
}
