...
#include "weapon_tazer.h"
#include "IEffects.h"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

ConVar    sk_plr_dmg_tazer	( "sk_plr_dmg_tazer","0");
ConVar    sk_npc_dmg_tazer	( "sk_npc_dmg_tazer","0");
...
IMPLEMENT_SERVERCLASS_ST(CWeaponTazer, DT_WeaponTazer)
	SendPropInt( SENDINFO( m_bActive ), 1, SPROP_UNSIGNED ),
END_SEND_TABLE()

//#ifndef HL2MP
LINK_ENTITY_TO_CLASS( weapon_tazer, CWeaponTazer );
PRECACHE_WEAPON_REGISTER( weapon_tazer );
//#endif

acttable_t CWeaponTazer::m_acttable[] = 
{
	{ ACT_MELEE_ATTACK1,	ACT_MELEE_ATTACK_SWING,	true },
	{ ACT_IDLE_ANGRY,		ACT_IDLE_ANGRY_MELEE,	true },
};

IMPLEMENT_ACTTABLE(CWeaponTazer);


BEGIN_DATADESC( CWeaponTazer )
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponTazer::CWeaponTazer( void )
{
...
//-----------------------------------------------------------------------------
float CWeaponTazer::GetDamageForActivity( Activity hitActivity )
{
	if ( ( GetOwner() != NULL ) && ( GetOwner()->IsPlayer() ) )
		return sk_plr_dmg_tazer.GetFloat();
	
	return sk_npc_dmg_tazer.GetFloat();
}
...
int CWeaponTazer::WeaponMeleeAttack1Condition( float flDot, float flDist )
{
	// Attempt to lead the target (needed because citizens can't hit manhacks with the crowbar!)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTazer::ImpactEffect( trace_t &traceHit )
{
...
void CWeaponTazer::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
void CWeaponTazer::SetTazerState( bool state )
{
	m_bActive = state;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponTazer::Holster( CBaseCombatWeapon *pSwitchingTo )
{
...
// Input  : &vecVelocity - 
//-----------------------------------------------------------------------------
void CWeaponTazer::Drop( const Vector &vecVelocity )
{
	SetTazerState( false );
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponTazer::GetTazerState( void )
{
	return m_bActive;
}
