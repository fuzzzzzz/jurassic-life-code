...
extern ConVar sk_plr_num_tranqualizer_pellets;

class CWeaponTranqualizer : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( CWeaponTranqualizer, CBaseHLCombatWeapon );
...
//	void WeaponIdle( void );
	...
	virtual void	ItemBusyFrame( void ); //jl addition
	...
	CWeaponTranqualizer(void);
};

IMPLEMENT_SERVERCLASS_ST(CWeaponTranqualizer, DT_WeaponTranqualizer)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_tranqualizer, CWeaponTranqualizer );
PRECACHE_WEAPON_REGISTER(weapon_tranqualizer);

BEGIN_DATADESC( CWeaponTranqualizer )
...
acttable_t	CWeaponTranqualizer::m_acttable[] = 
{
...
// Readiness activities (not aiming)
	{ ACT_IDLE_RELAXED,				ACT_IDLE_LINDS_RELAXED,		false },//never aims
	{ ACT_IDLE_STIMULATED,			ACT_IDLE_LINDS_STIMULATED,	false },
	{ ACT_IDLE_AGITATED,			ACT_IDLE_LINDS_AGITATED,		false },//always aims
...
IMPLEMENT_ACTTABLE(CWeaponTranqualizer);

void CWeaponTranqualizer::Precache( void )
{
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
float CWeaponTranqualizer::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponTranqualizer::GetMaxRestTime()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 1.5f;
...
//-----------------------------------------------------------------------------
float CWeaponTranqualizer::GetFireRate()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 0.8f;
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponTranqualizer::StartReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponTranqualizer::Reload( void )
{
	// Check that StartReload was called first
	if (!m_bInReload)
	{
		Warning("ERROR: Spas12 Reload called incorrectly!\n");
	}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::FinishReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::FillClip( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::Pump( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
//
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::DryFire( void )
{
	WeaponSound(EMPTY);
...
//
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::PrimaryAttack( void )
{
	...
	pPlayer->FireBullets( sk_plr_num_tranqualizer_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, true, true );
...
/*
//-----------------------------------------------------------------------------
// Purpose: Old Shotgun secondary attack, now replaced by the costumized zoom
// source code from the crossbow
//
//
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::SecondaryAttack( void )
{
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
// the weapon_crossbow begins here
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::SecondaryAttack( void )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::CheckZoomToggle( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::StopEffects( void )
{
	// Stop zooming
	if ( m_bInZoom )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::ItemBusyFrame( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::ItemPostFrame( void )
{
	//from crossbow source code
	// Allow zoom toggling
	CheckZoomToggle();
...
*/	
	...
	if ((m_bDelayedFire2 || pOwner->m_nButtons & IN_ATTACK2)&&(m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		// Fire underwater? make the bullet move more slow ?
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponTranqualizer::CWeaponTranqualizer( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponTranqualizer::ItemHolsterFrame( void )
{
	// Must be player held
...
//==================================================
/*
void CWeaponTranqualizer::WeaponIdle( void )
{
	//Only the player fires this way so we can cast
