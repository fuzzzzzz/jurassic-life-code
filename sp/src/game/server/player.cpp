...
//	DEFINE_CUSTOM_FIELD( m_Activity, ActivityDataOps() ),
	...
	//JL
	DEFINE_FIELD( m_iStress, FIELD_INTEGER),
	DEFINE_AUTO_ARRAY( m_nCaseItems, FIELD_EHANDLE ),
	DEFINE_FIELD( m_iNoteCount, FIELD_INTEGER),
	DEFINE_AUTO_ARRAY( m_iNotes, FIELD_INTEGER ),
	DEFINE_AUTO_ARRAY( m_bNoteRead, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bVguiUseFull, FIELD_BOOLEAN),
...
CBasePlayer::CBasePlayer( )
{
	...
	//JL(TH) : Set player stress to 0
	m_iStress = 0;
	m_iStressTarget = 0;
	m_fStressLastCheck = gpGlobals->curtime;

	SetThink(&CBasePlayer::Think);
	SetNextThink( gpGlobals->curtime );

	m_iNoteCount=0;
	m_bVguiUseFull=false;
...
{
	//JL(TH)
	if (m_iStress != m_iStressTarget)
	{
		if (m_fStressSpeed > 0.f)
		{
			float fPercent = (gpGlobals->curtime - m_fStressLastCheck)/m_fStressSpeed;
			m_iStress = m_iStressStart + (m_iStressTarget-m_iStressStart)*fPercent;
			if ((m_iStressTarget-m_iStressStart) < 0)
			{
				if (m_iStress < m_iStressTarget)
					m_iStress = m_iStressTarget;
			}else{
				if (m_iStress > m_iStressTarget)
					m_iStress = m_iStressTarget;
			}
...
// Purpose: 
//-----------------------------------------------------------------------------
static void CreateJLJeep( CBasePlayer *pPlayer )
{
	// Cheat to create a jeep in front of the player
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
	...
	CBaseEntity *pJeep = (CBaseEntity *)CreateEntityByName( "prop_vehicle_wrangler93" );
	if ( pJeep )
	{
		Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0,0,64);
		QAngle vecAngles( 0, pPlayer->GetAbsAngles().y - 90, 0 );
		pJeep->SetAbsOrigin( vecOrigin );
		pJeep->SetAbsAngles( vecAngles );
		pJeep->KeyValue( "model", "models/jl/jeep4.mdl" );
		pJeep->KeyValue( "solid", "6" );
		//pJeep->KeyValue( "targetname", "hl2buggy" );
		pJeep->KeyValue( "vehiclescript", "scripts/vehicles/wrangler93.txt" );
		DispatchSpawn( pJeep );
		pJeep->Activate();
		pJeep->Teleport( &vecOrigin, &vecAngles, NULL );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
static void CreateJLExplorer( CBasePlayer *pPlayer )
{
	// Cheat to create a jeep in front of the player
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
	//Tony; in sp sdk, we have prop_vehicle_hl2buggy; because episode 2 modified the jeep code to turn it into the jalopy instead of the regular buggy
	CBaseEntity *pJeep = (CBaseEntity *)CreateEntityByName( "prop_vehicle_explorer" );
	if ( pJeep )
	{
		Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0,0,64);
		QAngle vecAngles( 0, pPlayer->GetAbsAngles().y - 90, 0 );
		pJeep->SetAbsOrigin( vecOrigin );
		pJeep->SetAbsAngles( vecAngles );
		pJeep->KeyValue( "model", "models/jl/vehicles/FordExplorer.mdl" );
		pJeep->KeyValue( "solid", "6" );
		//pJeep->KeyValue( "targetname", "hl2buggy" );
		pJeep->KeyValue( "vehiclescript", "scripts/vehicles/fordexplorer.txt" );
		DispatchSpawn( pJeep );
		pJeep->Activate();
		pJeep->Teleport( &vecOrigin, &vecAngles, NULL );
	}
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 94:
		...
		//JL : impulse custom jl weapon arsenal -red
		GiveNamedItem( "weapon_flashlight" );
		GiveNamedItem( "weapon_rusticmachete" );
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	// JL Commands
	else if( stricmp( cmd, "setlogiccasevalue" ) == 0 )
	{
		CBaseEntity *pEntity = NULL;
		while ((pEntity = gEntList.FindEntityByName( pEntity, args[1] )) != NULL)
		{
			if (pEntity)
			{
				DevMsg("Entity : %s : %s : %s\n",pEntity->GetClassname(),args[1],args[2]);
				CLogicCase *prop = dynamic_cast<CLogicCase*>(pEntity);
				inputdata_t temp;				
				temp.value.SetString(MAKE_STRING(args[2]));
				temp.pActivator = pEntity;
				prop->InputValue(temp);
			}
		}
		return true;
	}/*else if( stricmp( cmd, "ent_fire" ) == 0 )
	{
		std::string command ("ent_fire ");
		command+=args[1];
		command+=" ";
		command+=args[2];
		engine->ServerCmd(command.c_str());
		return true;
	}*/else if( stricmp( cmd, "dropitem" ) == 0 )
	{
		int i = atoi(args[1]);
		if (GetWeapon(i-1))
			Weapon_Drop(GetWeapon(i-1),NULL,NULL);
		return true;
	}else if( stricmp( cmd, "primaryattack" ) == 0 )
	{
		int i = atoi(args[1]);
		if (GetWeapon(i-1))
		{
			GetWeapon(i-1)->PrimaryAttack();
			//Weapon_Switch(GetWeapon(i-1));
		}
		return true;
	}else if( stricmp( cmd, "switchweapon" ) == 0 )
	{
		int i = atoi(args[1]);
		if (GetWeapon(i-1))
			Weapon_Switch(GetWeapon(i-1));
		return true;
	}else if( stricmp( cmd, "switchInv" ) == 0 )
	{
		int i = atoi(args[1]);
		int j = atoi(args[2]);
		CBaseCombatWeapon *temp = GetWeapon(i);
		SetWeapon(i,GetWeapon(j));
		SetWeapon(j,temp);
		return true;
	}else if( stricmp( cmd, "switchCase" ) == 0 )
	{
		int i = atoi(args[1]);
		int j = atoi(args[2]);
		CBaseCombatWeapon *temp = GetCaseItem(i);
		SetCaseItem(i,GetCaseItem(j));
		SetCaseItem(j,temp);
		return true;
	}else if( stricmp( cmd, "switchInvToCase" ) == 0 )
	{
		int i = atoi(args[1]);
		int j = atoi(args[2]);
		DevMsg("switchInvToCase %d %d\n",i,j);
		CBaseCombatWeapon *temp = GetWeapon(i);
		SetWeapon(i,GetCaseItem(j));
		SetCaseItem(j,temp);
		if (GetActiveWeapon()==temp)
		{
			SwitchToNextBestWeapon(temp);
		}
		return true;
	}else if( stricmp( cmd, "switchCaseToInv" ) == 0 )
	{
		DevMsg("switchCaseToInv\n");
		int i = atoi(args[1]);
		int j = atoi(args[2]);
		if (GetActiveWeapon()==GetWeapon(j))
		{
			SwitchToNextBestWeapon(GetWeapon(j));
		}
		CBaseCombatWeapon *temp = GetCaseItem(i);
		SetCaseItem(i,GetWeapon(j));
		SetWeapon(j,temp);
		return true;
	}else if( stricmp( cmd, "exitVGui" ) == 0 )
	{
		//DevMsg("Exit VGui screen usefull\n");
		CBaseEntity *pEntity = NULL;
		while ((pEntity = gEntList.FindEntityByClassname( pEntity, "vgui_screen" )) != NULL)
		{
			if (pEntity)
			{
				//DevMsg("Entity : %s : %s : %s\n",pEntity->GetClassname(),args[1],args[2]);
				CVGuiScreen *screen = dynamic_cast<CVGuiScreen*>(pEntity);
				if (screen && screen->IsUseFull())
				{
					screen->UseFull(false);
					break;
				}
			}
		}
		return true;
	}else if( stricmp( cmd, "addnote" ) == 0 )
	{
		//m_iNotes[m_iNoteCount]=(m_iNoteCount+1);
		m_iNotes.Set(m_iNoteCount,m_iNoteCount+1);
		m_iNoteCount++;
		DevMsg("Note count %d\n",m_iNoteCount);
		DevMsg("Note value %d\n",m_iNotes.Get(m_iNoteCount));
		return true;
	}else if( stricmp( cmd, "readnote" ) == 0 )
	{
		DevMsg("ReadNote %d\n",atoi(args[1]));
		m_bNoteRead.Set(atoi(args[1]),true);
		return true;
	}
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	//JL(TH) : If is an item, not take ammo
	if (!pWeapon->IsItem() && Weapon_OwnsThisType( pWeapon->GetClassname(), pWeapon->GetSubType())) 
	{
	...
	//JL(TH) : Take Weapon if player has enough place in inventory
	else if (nItems<MAX_WEAPONS)
...
void CBasePlayer::UpdateClientData( void )
{
	...
	//JL(TH) : Jurassic Life stress effect
	// Radial blur Effect
	/*ConVar *pId  = cvar->FindVar( "jl_radialblur" );
	if (pId)
		pId->SetValue( GetStress() );
	pId  = cvar->FindVar( "jl_radialblur_sample" );
	if (pId)
		pId->SetValue( 20 );
	pId  = cvar->FindVar( "jl_radialblur_power" );
	if (pId)
		pId->SetValue( 0.5f );
	pId  = cvar->FindVar( "jl_radialblur_drawalpha" );
	if (pId)
		pId->SetValue( (float)GetStress()/100 );*/

	// Rumble effect
	//ViewPunch( QAngle( random->RandomFloat( -2*(float)GetStress()/100/2, -1*(float)GetStress()/100/2 ), random->RandomFloat( -2*(float)GetStress()/100/2, -1*(float)GetStress()/100/2 ), 0 ) );
	...
	if (GetStress()>40)
	{
		if (!IsInAVehicle())
		{
			...
			angles.x += random->RandomFloat( -1*(float)(GetStress()-40)/100, 1*(float)(GetStress()-40)/100 );
			angles.y += random->RandomFloat( -1*(float)(GetStress()-40)/100, 1*(float)(GetStress()-40)/100 );
			angles.z = 0;
			SnapEyeAngles( angles );
		}
	}
	...
	if (GetStress()>70)
	{
		float rand = random->RandomFloat( 0,1000 );
		DevMsg("Rand : %f\n",rand,NULL);
		if (rand < 1)
		{
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_SERVERCLASS_ST( CBasePlayer, DT_BasePlayer )
		...
		//JL(TH)
		SendPropInt		( SENDINFO( m_iStress ), 4, SPROP_UNSIGNED ),
		SendPropArray3	( SENDINFO_ARRAY3(m_nCaseItems), SendPropEHandle( SENDINFO_ARRAY(m_nCaseItems) ) ),
		SendPropInt		( SENDINFO( m_iNoteCount ), 4, SPROP_UNSIGNED ),
		SendPropArray3	( SENDINFO_ARRAY3(m_iNotes), SendPropInt( SENDINFO_ARRAY(m_iNotes) ) ),
		SendPropArray3	( SENDINFO_ARRAY3(m_bNoteRead), SendPropBool( SENDINFO_ARRAY(m_bNoteRead) ) ),		
		SendPropBool	( SENDINFO( m_bVguiUseFull ) ),
...
//[JL
void CBasePlayer::AddNote(int noteId)
{
	for (int i=0;i<m_iNoteCount;i++)
	{
		if (m_iNotes.Get(i)==noteId)
		{
			return;
		}
	}
	m_iNoteCount++;
	m_iNotes.Set(m_iNoteCount,noteId);	
	m_bNoteRead.Set(m_iNoteCount,false);
}

void CBasePlayer::SetStress(int i, float fSpeed)
{
	m_iStressTarget = i;
	m_iStressStart = m_iStress;
	m_fStressSpeed = fSpeed;
	m_fStressLastCheck = gpGlobals->curtime;
}
//JL]
