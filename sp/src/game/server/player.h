...
public:
	...
	CNetworkVar(int, m_iNoteCount);
	CNetworkArray(int, m_iNotes, MAX_JL_NOTES);
	CNetworkArray(bool, m_bNoteRead, MAX_JL_NOTES);	
	CNetworkVar(bool, m_bVguiUseFull);

	int						GetStress() { return m_iStress;}
	void					SetStress(int i, float fSpeed = 2.f);
	CBaseCombatWeaponHandle GetCaseItem(int i) { return m_nCaseItems[i]; };
	void					SetCaseItem(int i, CBaseCombatWeaponHandle n) { m_nCaseItems.Set(i,n) ; };
 	void					SetVguiUseFull(bool used) { m_bVguiUseFull=used; }
	bool					GetVguiUseFull() { return m_bVguiUseFull; }
