...
//JL
#define MOSQUITOPARTICLE_ENTITYNAME	"env_mosquito"
...
IMPLEMENT_SERVERCLASS_ST( MosquitoParticle, DT_MosquitoParticle )
	SendPropFloat	(SENDINFO(m_flSpawnRate), 8, 0, 1, 1024),
	SendPropFloat	(SENDINFO(m_flParticleLifetime), 16, SPROP_ROUNDUP, 0.1, 100),
	SendPropFloat	(SENDINFO(m_flStartSize), -1, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_flEndSize), -1, SPROP_NOSCALE),
...
LINK_ENTITY_TO_CLASS( env_mosquito, MosquitoParticle );

BEGIN_DATADESC( MosquitoParticle )
...
MosquitoParticle::MosquitoParticle( void )
{
	m_flSpawnRate			= 100.0f;
	m_flParticleLifetime	= 1.0f;
	m_flStartSize			= 1.0f;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void MosquitoParticle::Spawn( void )
{
	BaseClass::Spawn();
...
//-----------------------------------------------------------------------------
// Purpose: 
// Output : MosquitoParticle*
//-----------------------------------------------------------------------------
MosquitoParticle *MosquitoParticle::CreateMosquitoParticle()
{
	CBaseEntity *pEnt = CreateEntityByName( MOSQUITOPARTICLE_ENTITYNAME );
	
	if ( pEnt )
	{
		MosquitoParticle *pSpore = dynamic_cast<MosquitoParticle*>(pEnt);
...
void MosquitoParticle::InputEnable( inputdata_t &inputdata )
{
	m_bDontRemove = true;
	m_bDisabled = false;
	m_bEmit = true;
...
void MosquitoParticle::InputDisable( inputdata_t &inputdata )
{
	m_bDontRemove = true;
	m_bDisabled = true;
	m_bEmit = false;
