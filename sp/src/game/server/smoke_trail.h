...
class MosquitoParticle : public CBaseParticleEntity
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( MosquitoParticle, CBaseParticleEntity );
	DECLARE_SERVERCLASS();

	MosquitoParticle( void );
	void Spawn( void );

	static MosquitoParticle*		CreateMosquitoParticle();

	void InputEnable( inputdata_t &inputdata );
	void InputDisable( inputdata_t &inputdata );

	//player chase insects (flies and mosquitos) -red
	bool InsectsTouch( CBasePlayer *pPlayer );
//Data members
public:
