...
void CBaseCombatWeapon::Precache( void )
{
	...
	if ( ReadWeaponDataFromFileForSlot( filesystem, GetClassname(), &m_hWeaponFileInfo, GetEncryptionKey() ) )
	{
		...
		//JL(TH)
		m_bIsItem = GetWpnData().bIsItem;
		m_iItemType = GetWpnData().iItemType;
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	//JL(TH) : define weapon jl field
	DEFINE_FIELD( m_bIsItem, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_iItemType, FIELD_INTEGER ),
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalWeaponData )
	...
	//JL(TH) : define send prop jl weapon data
	SendPropBool( SENDINFO(m_bIsItem ) ),
	SendPropInt( SENDINFO(m_iItemType ) ),
	...
	//JL(TH) : define receive jl weapon data
	RecvPropBool( RECVINFO(m_bIsItem )),
	RecvPropInt( RECVINFO(m_iItemType )),
