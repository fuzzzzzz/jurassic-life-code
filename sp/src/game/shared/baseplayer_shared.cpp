...
void CBasePlayer::ItemPreFrame()
{
	...
	//JL
	//JL(TH) : Drop weapon
	if (m_nButtons & IN_DROP && !(m_afButtonLast & IN_DROP))
	{	
#if !defined( CLIENT_DLL )
		if (strcmp(GetActiveWeapon()->GetClassname(),"weapon_flashlight"))
		{
			DevMsg("Drop weapon \n");		
...
void CBasePlayer::ItemPostFrame()
{
	...
	else
	{
		if ( GetActiveWeapon() && (!IsInAVehicle() || UsingStandardWeaponsInVehicle()) )
		{
			...
			{
				//JL(TH) : If is sprinting, not update weapon
				#if !defined( CLIENT_DLL )
				//if( ((CHL2_Player*)this)->IsSprinting() == false )
				#endif
