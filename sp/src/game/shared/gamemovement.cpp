...
//JL(TH) : Camera Bob parameters
ConVar cl_viewbob_enabled		( "cl_viewbob_enabled", "1", 0, "Oscillation Toggle", true, 0, true, 1 );
ConVar cl_viewbob_timer_walk	( "cl_viewbob_timer_walk", "7.3", 0, "Speed of Oscillation");
ConVar cl_viewbob_scale_walk	( "cl_viewbob_scale_walk", "0.005", 0, "Magnitude of Oscillation");
ConVar cl_viewbob_timer_sprint	( "cl_viewbob_timer_sprint", "19", 0, "Speed of Oscillation");
ConVar cl_viewbob_scale_sprint	( "cl_viewbob_scale_sprint", "0.018", 0, "Magnitude of Oscillation");
...
void CGameMovement::WalkMove( void )
{
	...
	{
		float timer = cl_viewbob_timer_walk.GetFloat();
		float scale = cl_viewbob_scale_walk.GetFloat();

/*
if ( cl_viewbob_enabled.GetInt() == 1 && !engine->IsPaused() )
	{
		float xoffset = sin( gpGlobals->curtime * cl_viewbob_timer_walk.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale_walk.GetFloat() / 100;
		float yoffset = sin( 2 * gpGlobals->curtime * cl_viewbob_timer_walk.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale_walk.GetFloat() / 400;
...
*/
		...
		if (pHLPlayer && pHLPlayer->IsSprinting())
		{
			timer = cl_viewbob_timer_sprint.GetFloat();
			scale = cl_viewbob_scale_sprint.GetFloat();
		}
	#endif

		float xoffset = sin( gpGlobals->curtime * timer ) * 
			player->GetAbsVelocity().Length() * scale / 100;
		float yoffset = sin( 2 * gpGlobals->curtime * timer ) * 
			player->GetAbsVelocity().Length() * 2.0 * scale / 400;
