...
FileWeaponInfo_t::FileWeaponInfo_t()
{
	...
	//JL(TH) : by default weapon is not an item
	bIsItem = false;
	iItemType = 0;
	fPosZoom = 50;
	fPosX = 0;
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	//JL(TH) : get if an weapon is an item and position in inventory preview
	bIsItem = ( pKeyValuesData->GetInt( "isitem", 0 ) != 0 ) ? true : false;
	fPosZoom = pKeyValuesData->GetFloat("pos_zoom",50);
	fPosX = pKeyValuesData->GetFloat("pos_x");
	fPosY = pKeyValuesData->GetFloat("pos_y");
	fPosZ = pKeyValuesData->GetFloat("pos_z");
	fPosTop = pKeyValuesData->GetFloat("pos_top");
	fPosLeft = pKeyValuesData->GetFloat("pos_left");

	iItemType = pKeyValuesData->GetInt( "itemType", 0 );
