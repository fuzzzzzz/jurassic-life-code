...
#include "JL_BlurFilter_vs20.inc"
#include "JL_BlurFilter_ps20.inc"
#include "JL_BlurFilter_ps20b.inc"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

BEGIN_VS_SHADER_FLAGS( JL_BlurFilterY, "Help for JL_BlurFilterY", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			// Pre-cache shaders
			DECLARE_STATIC_VERTEX_SHADER( jl_blurfilter_vs20 );
			SET_STATIC_VERTEX_SHADER( jl_blurfilter_vs20 );
			
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() || g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders() )
			{
				DECLARE_STATIC_PIXEL_SHADER( jl_blurfilter_ps20b );
#ifndef _X360
				SET_STATIC_PIXEL_SHADER_COMBO( APPROX_SRGB_ADAPTER, bForceSRGBReadAndWrite );
#endif
				SET_STATIC_PIXEL_SHADER( jl_blurfilter_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( jl_blurfilter_ps20 );
				SET_STATIC_PIXEL_SHADER( jl_blurfilter_ps20 );
			}
...
//			dY *= 0.4;
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( jl_blurfilter_ps20 );
			SET_DYNAMIC_VERTEX_SHADER( jl_blurfilter_ps20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() || g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_blurfilter_ps20b );
				SET_DYNAMIC_PIXEL_SHADER( jl_blurfilter_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_blurfilter_ps20 );
				SET_DYNAMIC_PIXEL_SHADER( jl_blurfilter_ps20 );
			}
		}
		Draw();
	}
