...
#include "JL_worldvertextransition.inc"
#include "JL_worldvertextransition_vs14.inc"
#include "JL_worldvertextransition_seamless.inc"
#include "JL_lightmappedgeneric_vs11.inc"
#include "JL_writevertexalphatodestalpha_vs11.inc"
#include "worldvertextransition_dx8_helper.h"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

DEFINE_FALLBACK_SHADER( JL_WorldVertexTransition, JL_WorldVertexTransition_DX8 )

//already defined into baseVSShader.cpp --red
extern ConVar mat_fullbright; //( "mat_fullbright","0", FCVAR_CHEAT );

BEGIN_VS_SHADER( JL_WorldVertexTransition_DX8,
			  "Help for JL_WorldVertexTransition_DX8" )
	...
	SHADER_FALLBACK
	{
		if ( IsPC() && g_pHardwareConfig->GetDXSupportLevel() < 80 )
			return "JL_WorldVertexTransition_DX6";
		return 0;
	}
	...
	void WriteVertexAlphaToDestAlpha( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow )
	{
		if( pShaderShadow )
		{
			...
			jl_writevertexalphatodestalpha_vs11_Static_Index vshIndex;
			pShaderShadow->SetVertexShader( "jl_writevertexalphatodestalpha_vs11", vshIndex.GetIndex() );
		
			pShaderShadow->SetPixelShader( "JL_writevertexalphatodestalpha_ps11" );
			pShaderShadow->VertexShaderVertexFormat( 
				VERTEX_POSITION | VERTEX_COLOR, 2, 0, 0 );
		}
		else
		{
			jl_writevertexalphatodestalpha_vs11_Dynamic_Index vshIndex;
	...
	SHADER_DRAW
	{
		...
		else if( !params[BUMPMAP]->IsTexture() || !g_pConfig->UseBumpmapping() )
		{
			...
			SHADOW_STATE
			{
				...
				if ( !bSupports14 )
				{
					jl_worldvertextransition_Static_Index vshIndex;
					pShaderShadow->SetVertexShader( "JL_WorldVertexTransition", vshIndex.GetIndex() );

					int pshIndex = bDetail ? 1 : 0;
					pShaderShadow->SetPixelShader( "JL_WorldVertexTransition", pshIndex );
				}
				else
				{
					jl_worldvertextransition_vs14_Static_Index vshIndex;
					pShaderShadow->SetVertexShader( "JL_WorldVertexTransition_vs14", vshIndex.GetIndex() );
			...
			DYNAMIC_STATE
			{
				...
				if ( !bSupports14 )
				{
					jl_worldvertextransition_Dynamic_Index vshIndex;
					vshIndex.SetDOWATERFOG( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					pShaderAPI->SetVertexShaderIndex( vshIndex.GetIndex() );
				}
				else
				{
					jl_worldvertextransition_vs14_Dynamic_Index vshIndex;
					vshIndex.SetDOWATERFOG( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					pShaderAPI->SetVertexShaderIndex( vshIndex.GetIndex() );
				}
			}
		...
		else
		{
			...
			else
			{
				...
				SHADOW_STATE
				{
					...
					pShaderShadow->SetPixelShader( "JL_WorldVertexTransition_BlendBase2" );
					FogToFogColor();
				}
				DYNAMIC_STATE
				{
