...
#include "JL_cable_vs20.inc"
#include "JL_cable_ps20.inc"
#include "JL_cable_ps20b.inc"
...
DEFINE_FALLBACK_SHADER( JL_Cable, JL_Cable_DX9 )

BEGIN_VS_SHADER( JL_Cable_DX9, 
			  "Help for JL_Cable shader" )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( jl_cable_vs20 );
			SET_STATIC_VERTEX_SHADER( jl_cable_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( jl_cable_ps20b );
				SET_STATIC_PIXEL_SHADER( jl_cable_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( jl_cable_ps20 );
				SET_STATIC_PIXEL_SHADER( jl_cable_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( jl_cable_vs20 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
			SET_DYNAMIC_VERTEX_SHADER( jl_cable_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_cable_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
				SET_DYNAMIC_PIXEL_SHADER( jl_cable_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_cable_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER( jl_cable_ps20 );
			}
		}
		Draw();
	}
