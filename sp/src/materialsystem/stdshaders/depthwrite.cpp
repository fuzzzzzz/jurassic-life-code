...
#include "JL_depthwrite_ps20.inc"
#include "JL_depthwrite_ps20b.inc"
#include "JL_depthwrite_vs20.inc"

#if !defined( _X360 )
#include "JL_depthwrite_ps30.inc"
#include "JL_depthwrite_vs30.inc"
#endif

BEGIN_VS_SHADER_FLAGS( JL_DepthWrite, "Help for JL_Depth Write", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER( jl_depthwrite_vs20 );
				...
				SET_STATIC_VERTEX_SHADER( jl_depthwrite_vs20 );
				...
				if ( bAlphaClip || g_pHardwareConfig->PlatformRequiresNonNullPixelShaders() || nColorDepth )
				{
					...
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_depthwrite_ps20b );
						SET_STATIC_PIXEL_SHADER_COMBO( COLOR_DEPTH, nColorDepth );
						SET_STATIC_PIXEL_SHADER( jl_depthwrite_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_depthwrite_ps20 );
						SET_STATIC_PIXEL_SHADER_COMBO( COLOR_DEPTH, nColorDepth );
						SET_STATIC_PIXEL_SHADER( jl_depthwrite_ps20 );
					}
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_STATIC_VERTEX_SHADER( jl_depthwrite_vs30 );
				SET_STATIC_VERTEX_SHADER_COMBO( ONLY_PROJECT_POSITION, 0 ); //360 only combo, and this is a PC path
				SET_STATIC_VERTEX_SHADER_COMBO( COLOR_DEPTH, nColorDepth );
				SET_STATIC_VERTEX_SHADER( jl_depthwrite_vs30 );

				pShaderShadow->EnableTexture( SHADER_SAMPLER0, true );
				pShaderShadow->EnableSRGBRead( SHADER_SAMPLER0, true );

				DECLARE_STATIC_PIXEL_SHADER( jl_depthwrite_ps30 );
				SET_STATIC_PIXEL_SHADER_COMBO( COLOR_DEPTH, nColorDepth );
				SET_STATIC_PIXEL_SHADER( jl_depthwrite_ps30 );
			}
#endif
		}
		DYNAMIC_STATE
		{
...
#endif
			{
				jl_depthwrite_vs20_Dynamic_Index vshIndex;
				...
				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( ALPHACLIP, bAlphaClip );
					SET_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( ALPHACLIP, bAlphaClip );
					SET_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps20 );
				}
			}
#ifndef _X360
			else // 3.0 shader case (PC only)
			{
				...
				jl_depthwrite_vs30_Dynamic_Index vshIndex;
				vshIndex.SetSKINNING( pShaderAPI->GetCurrentNumBones() > 0 );
				...
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( ALPHACLIP, bAlphaClip );
				SET_DYNAMIC_PIXEL_SHADER( jl_depthwrite_ps30 );
			}
#endif
