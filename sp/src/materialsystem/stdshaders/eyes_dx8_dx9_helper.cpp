...
#include "JL_Eyes.inc"
#include "JL_eyes_flashlight_vs11.inc"
#include "JL_eyes_flashlight_ps11.inc"


#ifdef STDSHADER_DX9_DLL_EXPORT

#include "JL_eyes_vs20.inc"
#include "JL_eyes_ps20.inc"
#include "JL_eyes_ps20b.inc"
#include "JL_eyes_flashlight_vs20.inc"
#include "JL_eyes_flashlight_ps20.inc"
#include "JL_eyes_flashlight_ps20b.inc"

#ifndef _X360
#include "JL_eyes_vs30.inc"
#include "JL_eyes_ps30.inc"
#include "JL_eyes_flashlight_vs30.inc"
#include "JL_eyes_flashlight_ps30.inc"
#endif
	...
	if( pShaderShadow )
	{
		...
		if ( bDX9 )
		{
...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER( jl_eyes_flashlight_vs20 );
				SET_STATIC_VERTEX_SHADER( jl_eyes_flashlight_vs20 );

				if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps20b );
					SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode );
					SET_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps20b );
				}
				else
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps20 );
					SET_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_STATIC_VERTEX_SHADER( jl_eyes_flashlight_vs30 );
				SET_STATIC_VERTEX_SHADER( jl_eyes_flashlight_vs30 );

				DECLARE_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps30 );
				SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode );
				SET_STATIC_PIXEL_SHADER( jl_eyes_flashlight_ps30 );
			}
#endif
...
#endif
		{
			...
			jl_eyes_flashlight_vs11_Static_Index	vshIndex;
			pShaderShadow->SetVertexShader( "JL_eyes_flashlight_vs11", vshIndex.GetIndex() );

			jl_eyes_flashlight_ps11_Static_Index	pshIndex;
			pShaderShadow->SetPixelShader( "JL_eyes_flashlight_ps11", pshIndex.GetIndex() );
		}
...
#endif
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_eyes_flashlight_vs20 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( jl_eyes_flashlight_vs20 );
			}
#ifndef _X360
			else
			{
				...
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_eyes_flashlight_vs30 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING, pShaderAPI->IsHWMorphingEnabled() );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( jl_eyes_flashlight_vs30 );
			}
#endif
...
//			pShaderAPI->SetPixelShaderConstant( 0, vPSConst, 1 );
...
#endif
			{
				if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps20b );
					...
					SET_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps20b );

					SetDepthFlashlightParams( pShader, pShaderAPI, worldToTexture, flashlightState );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, flashlightState.m_bEnableShadows && ( pFlashlightDepthTexture != NULL ) );
				SET_DYNAMIC_PIXEL_SHADER( jl_eyes_flashlight_ps30 );
...
#endif
		{
			jl_eyes_flashlight_vs11_Dynamic_Index vshIndex;
			vshIndex.SetDOWATERFOG( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
			vshIndex.SetSKINNING( pShaderAPI->GetCurrentNumBones() > 0 );
			pShaderAPI->SetVertexShaderIndex( vshIndex.GetIndex() );

			jl_eyes_flashlight_ps11_Dynamic_Index pshIndex;
	...
	SHADOW_STATE
	{
		...
		if ( bDX9 )
		{
...
#endif
			{
				...
				DECLARE_STATIC_VERTEX_SHADER( jl_eyes_vs20 );
				...
				SET_STATIC_VERTEX_SHADER( jl_eyes_vs20 );

				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_eyes_ps20b );
					SET_STATIC_PIXEL_SHADER( jl_eyes_ps20b );
				}
				else
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_eyes_ps20 );
					SET_STATIC_PIXEL_SHADER( jl_eyes_ps20 );
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_STATIC_VERTEX_SHADER( jl_eyes_vs30 );
				SET_STATIC_VERTEX_SHADER_COMBO( HALFLAMBERT, IS_FLAG_SET( MATERIAL_VAR_HALFLAMBERT ) );
				SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[info.m_nIntro]->GetIntValue() ? 1 : 0 );
				SET_STATIC_VERTEX_SHADER( jl_eyes_vs30 );

				DECLARE_STATIC_PIXEL_SHADER( jl_eyes_ps30 );
				SET_STATIC_PIXEL_SHADER( jl_eyes_ps30 );
			}
#endif
			// On DX9, get the gamma read and write correct
...
#endif
		{
			jl_eyes_Static_Index vshIndex;
			vshIndex.SetHALF_LAMBERT( IS_FLAG_SET( MATERIAL_VAR_HALFLAMBERT ) );
			pShaderShadow->SetVertexShader( "JL_Eyes", vshIndex.GetIndex() );

			pShaderShadow->SetPixelShader( "JL_Eyes_Overbright2" );
		}
	...
	DYNAMIC_STATE
	{
		...
		if( bDX9 )
		{
...
#endif
			{
				...
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_eyes_vs20 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				...
				SET_DYNAMIC_VERTEX_SHADER( jl_eyes_vs20 );
			}
#ifndef _X360
			else
			{
				...
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_eyes_vs30 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DYNAMIC_LIGHT, lightState.HasDynamicLight() );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( STATIC_LIGHT,  lightState.m_bStaticLight  ? 1 : 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING, pShaderAPI->IsHWMorphingEnabled() );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( jl_eyes_vs30 );
			}
#endif
...
#endif
			{
				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( jl_eyes_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER( jl_eyes_ps20 );
				}
			}
#ifndef _X360
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( jl_eyes_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, pShaderAPI->ShouldWriteDepthToDestAlpha() );
				SET_DYNAMIC_PIXEL_SHADER( jl_eyes_ps30 );
			}
#endif
...
#endif
		{
			jl_eyes_Dynamic_Index vshIndex;
			vshIndex.SetDOWATERFOG( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
			vshIndex.SetSKINNING( pShaderAPI->GetCurrentNumBones() > 0 );
