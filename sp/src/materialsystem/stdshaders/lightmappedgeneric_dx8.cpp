...
#include "jl_lightmappedgeneric_vs11.inc"
#include "jl_unlitgeneric_vs11.inc"
#include "JL_worldvertextransition_seamless.inc"
...
DEFINE_FALLBACK_SHADER( JL_LightmappedGeneric, JL_LightmappedGeneric_DX8 )

BEGIN_VS_SHADER( JL_LightmappedGeneric_DX8,
			  "Help for JL_LightmappedGeneric_DX8" )
	...
	SHADER_INIT_PARAMS()
	{
		...
		if( params[SEAMLESS_SCALE]->IsDefined() && params[SEAMLESS_SCALE]->GetFloatValue() != 0.0f )
		{
			...
			{
				Warning( "Can't use $bumpmap with $seamless_scale for jl_lightmappedgeneric_dx8.  Implicitly disabling $bumpmap: %s\n", pMaterialName );
				params[BUMPMAP]->SetUndefined();
			}
			if( params[ENVMAP]->IsDefined() )
			{
				Warning( "Can't use $envmap with $seamless_scale for jl_lightmappedgeneric_dx8. Implicitly disabling $envmap: %s\n", pMaterialName );
				params[ENVMAP]->SetUndefined();
			}
		}
	...
	SHADER_FALLBACK
	{
		if ( IsPC() && g_pHardwareConfig->GetDXSupportLevel() < 80)
			return "JL_LightmappedGeneric_DX6";

		if ( IsPC() && g_pHardwareConfig->PreferReducedFillrate() )
			return "JL_LightmappedGeneric_NoBump_DX8";
	...
	inline const char *GetPixelShaderName( IMaterialVar** params, bool bBumpedEnvMap )
	{
		...
		{
			// Unmasked
			"JL_LightmappedGeneric_EnvMapV2",
			"JL_LightmappedGeneric_SelfIlluminatedEnvMapV2",

			"JL_LightmappedGeneric_BaseAlphaMaskedEnvMapV2",
			"JL_LightmappedGeneric_SelfIlluminatedEnvMapV2",

			// Env map mask
			"JL_LightmappedGeneric_MaskedEnvMapV2",
			"JL_LightmappedGeneric_SelfIlluminatedMaskedEnvMapV2",
		...
		if (!params[BASETEXTURE]->IsTexture())
		{
			if (params[ENVMAP]->IsTexture() && !bBumpedEnvMap )
			{
				if (!params[ENVMAPMASK]->IsDefined() )
				{
					return "JL_LightmappedGeneric_EnvmapNoTexture";
				}
				else
				{
					return "JL_LightmappedGeneric_MaskedEnvmapNoTexture";
				}
			}
			else
			{
				return "JL_LightmappedGeneric_NoTexture";
			}
		}
 		else
		{
			...
			{
				if (IS_FLAG_SET(MATERIAL_VAR_SELFILLUM))
					return "JL_LightmappedGeneric_SelfIlluminated";
				else
					return "JL_LightmappedGeneric";
			}
		}
	}
#endif
	...
	void DrawUnbumpedUsingVertexShader( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, bool bBumpedEnvMap )
	{
		...
		if ( hasEnvmap || hasBaseTexture || hasVertexColor || !bBumpedEnvMap )
		{
			SHADOW_STATE
			{
				...
				jl_lightmappedgeneric_vs11_Static_Index vshIndex;
				...
				pShaderShadow->SetVertexShader( "JL_LightmappedGeneric_vs11", vshIndex.GetIndex() );
			...
			DYNAMIC_STATE
			{
				...
				jl_lightmappedgeneric_vs11_Dynamic_Index vshIndex;
	...
	void DrawDetailNoEnvmap( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, bool doSelfIllum )
	{
		SHADOW_STATE
		{
			...
			if (!params[BASETEXTURE]->IsTexture())
			{
				pShaderShadow->SetPixelShader("JL_LightmappedGeneric_DetailNoTexture");
			}
			else
			{
				if (!IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) || (!doSelfIllum))
				{
					pShaderShadow->SetPixelShader("JL_LightmappedGeneric_Detail");
				}
				else
				{
					pShaderShadow->SetPixelShader("JL_LightmappedGeneric_DetailSelfIlluminated");
				}
			}
			DefaultFog();
		}
		...
		bool usingBaseTexture, bool usingBaseAlphaEnvmapMask )
	{
		static char const* s_pPixelShaders[] = 
		{
			"JL_LightmappedGeneric_AddEnvmapNoTexture",
			"JL_LightmappedGeneric_AddEnvmapMaskNoTexture",
		};

		if ( !usingMask && usingBaseTexture && usingBaseAlphaEnvmapMask )
			return "JL_LightmappedGeneric_AddBaseAlphaMaskedEnvMap";
	...
	void DrawAdditiveEnvmap( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow )
	{
		...
		SHADOW_STATE
		{
			...
			s_pShaderShadow->SetVertexShader( "JL_LightmappedGeneric_vs11", vshIndex.GetIndex() );
	...
	void DrawUnbumpedSeamlessUsingVertexShader( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow )
	{
		...
		SHADOW_STATE
		{
			...
			jl_worldvertextransition_seamless_Static_Index vshIndex;
			pShaderShadow->SetVertexShader( "JL_WorldVertexTransition_Seamless", vshIndex.GetIndex() );

			int pshIndex = 0;
			pShaderShadow->SetPixelShader( "JL_WorldVertexTransition_Seamless", pshIndex );
		...
		DYNAMIC_STATE
		{
			...
			jl_worldvertextransition_seamless_Dynamic_Index vshIndex;
			vshIndex.SetDOWATERFOG( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
			pShaderAPI->SetVertexShaderIndex( vshIndex.GetIndex() );
		}
		Draw();
...
//-----------------------------------------------------------------------------
BEGIN_INHERITED_SHADER( JL_LightmappedGeneric_NoBump_DX8, JL_LightmappedGeneric_DX8,
			  "Help for JL_LightmappedGeneric_NoBump_DX8" )
