...
#if !defined( _X360 )
#include "JL_shadowmodel_ps20.inc"
#include "JL_shadowmodel_vs20.inc"
#endif

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

DEFINE_FALLBACK_SHADER( JL_ShadowModel, JL_ShadowModel_DX9 )


#if !defined( _X360 ) //not used for anything at time of 360 ship, and we want to avoid storing/loading assembly shaders

//PC version
BEGIN_VS_SHADER_FLAGS( JL_ShadowModel_DX9, "Help for JL_ShadowModel", SHADER_NOT_EDITABLE )
...
SHADER_DRAW
{
	SHADOW_STATE
	{
		...
		DECLARE_STATIC_VERTEX_SHADER( jl_shadowmodel_vs20 );
		SET_STATIC_VERTEX_SHADER( jl_shadowmodel_vs20 );

		DECLARE_STATIC_PIXEL_SHADER( jl_shadowmodel_ps20 );
		SET_STATIC_PIXEL_SHADER( jl_shadowmodel_ps20 );
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER( jl_shadowmodel_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
		SET_DYNAMIC_VERTEX_SHADER( jl_shadowmodel_vs20 );

		DECLARE_DYNAMIC_PIXEL_SHADER( jl_shadowmodel_ps20 );
		SET_DYNAMIC_PIXEL_SHADER( jl_shadowmodel_ps20 );
	}
	Draw( );
}
END_SHADER
