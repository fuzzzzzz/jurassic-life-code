...
#include "JL_sky_vs20.inc"
#include "JL_sky_ps20.inc"
#include "JL_sky_ps20b.inc"
#include "JL_sky_hdr_compressed_ps20.inc"
#include "JL_sky_hdr_compressed_ps20b.inc"
#include "JL_sky_hdr_compressed_rgbs_ps20.inc"
#include "JL_sky_hdr_compressed_rgbs_ps20b.inc"

#include "convar.h"

static ConVar mat_use_compressed_hdr_textures( "mat_use_compressed_hdr_textures", "1" );

DEFINE_FALLBACK_SHADER( JL_Sky, JL_Sky_HDR_DX9 )

BEGIN_VS_SHADER( JL_Sky_HDR_DX9, "Help for JL_Sky_HDR_DX9 shader" )
	BEGIN_SHADER_PARAMS
...
//			pShaderShadow->EnableAlphaWrites( true );
			...
			DECLARE_STATIC_VERTEX_SHADER( jl_sky_vs20 );
			SET_STATIC_VERTEX_SHADER( jl_sky_vs20 );
				 ...
				 mat_use_compressed_hdr_textures.GetBool() )
			{
				...
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20b );
					SET_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20b );
				}
				else
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20 );
					SET_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20 );
				}
			}
			else
			{
				if (params[HDRCOMPRESSEDTEXTURE0]->IsDefined())
				{
					...
					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20b );
						SET_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20 );
						SET_STATIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20 );
					}
				}
				else
				{
					...
					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_sky_ps20b );
						SET_STATIC_PIXEL_SHADER( jl_sky_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_sky_ps20 );
						SET_STATIC_PIXEL_SHADER( jl_sky_ps20 );
					}
				}
			}
		...
		DYNAMIC_STATE
		{
			DECLARE_DYNAMIC_VERTEX_SHADER( jl_sky_vs20 );
			SET_DYNAMIC_VERTEX_SHADER( jl_sky_vs20 );
				...
				)
			{
				...
				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20 );
					SET_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_rgbs_ps20 );
				}
			}
			else
			{
				...
				if (params[HDRCOMPRESSEDTEXTURE0]->IsDefined() )
				{
					...
					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20b );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, pShaderAPI->ShouldWriteDepthToDestAlpha() );
						SET_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20 );
						SET_DYNAMIC_PIXEL_SHADER( jl_sky_hdr_compressed_ps20 );
					}
				...
				else
				{
					...
					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_ps20b );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, pShaderAPI->ShouldWriteDepthToDestAlpha() );
						SET_DYNAMIC_PIXEL_SHADER( jl_sky_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_sky_ps20 );
						SET_DYNAMIC_PIXEL_SHADER( jl_sky_ps20 );
					}
				}
			}
