...
#include "JL_teeth_vs20.inc"
#include "JL_teeth_flashlight_vs20.inc"
#include "JL_teeth_bump_vs20.inc"
#include "JL_teeth_ps20.inc"
#include "JL_teeth_ps20b.inc"
#include "JL_teeth_flashlight_ps20.inc"
#include "JL_teeth_flashlight_ps20b.inc"
#include "JL_teeth_bump_ps20.inc"
#include "JL_teeth_bump_ps20b.inc"

#ifndef _X360
#include "JL_teeth_vs30.inc"
#include "JL_teeth_ps30.inc"
#include "JL_teeth_bump_vs30.inc"
#include "JL_teeth_bump_ps30.inc"
#include "JL_teeth_flashlight_vs30.inc"
#include "JL_teeth_flashlight_ps30.inc"
#endif

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

DEFINE_FALLBACK_SHADER( JL_Teeth, JL_Teeth_DX9 )

extern ConVar r_flashlight_version2;
BEGIN_VS_SHADER( JL_Teeth_DX9, "Help for JL_Teeth_DX9" )
		...
		SHADOW_STATE
		{
			...
			if ( hasBump )
			{
...
#endif
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( jl_teeth_bump_vs20 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER_COMBO( USE_STATIC_CONTROL_FLOW, bUseStaticControlFlow );
					SET_STATIC_VERTEX_SHADER( jl_teeth_bump_vs20 );

					// ps_2_b version which does phong
					if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_teeth_bump_ps20b );
						SET_STATIC_PIXEL_SHADER( jl_teeth_bump_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_teeth_bump_ps20 );
						SET_STATIC_PIXEL_SHADER( jl_teeth_bump_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( jl_teeth_bump_vs30 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER( jl_teeth_bump_vs30 );

					DECLARE_STATIC_PIXEL_SHADER( jl_teeth_bump_ps30 );
					SET_STATIC_PIXEL_SHADER( jl_teeth_bump_ps30 );
				}
#endif
			}
			else
			{
...
#endif
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( jl_teeth_vs20 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER_COMBO( USE_STATIC_CONTROL_FLOW, bUseStaticControlFlow );
					SET_STATIC_VERTEX_SHADER( jl_teeth_vs20 );

					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_teeth_ps20b );
						SET_STATIC_PIXEL_SHADER( jl_teeth_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_teeth_ps20 );
						SET_STATIC_PIXEL_SHADER( jl_teeth_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( jl_teeth_vs30 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER( jl_teeth_vs30 );

					DECLARE_STATIC_PIXEL_SHADER( jl_teeth_ps30 );
					SET_STATIC_PIXEL_SHADER( jl_teeth_ps30 );
				}
#endif
			}
		...
		DYNAMIC_STATE
		{
			...
			{	
...
#endif
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_bump_vs20 );
					...
					SET_DYNAMIC_VERTEX_SHADER( jl_teeth_bump_vs20 );
					...
					if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						...
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps20b );
						...
						SET_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps20 );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( NUM_LIGHTS, lightState.m_nNumLights );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( AMBIENT_LIGHT, lightState.m_bAmbientLight ? 1 : 0 );
						SET_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_bump_vs30 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( STATIC_LIGHT,  lightState.m_bStaticLight  ? 1 : 0 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING,  pShaderAPI->IsHWMorphingEnabled() );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
					SET_DYNAMIC_VERTEX_SHADER( jl_teeth_bump_vs30 );

					Vector4D vSpecExponent;
					vSpecExponent[3] = params[PHONGEXPONENT]->GetFloatValue();
					pShaderAPI->SetPixelShaderConstant( PSREG_EYEPOS_SPEC_EXPONENT, vSpecExponent.Base(), 1 );

					DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps30 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( NUM_LIGHTS,  lightState.m_nNumLights );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( AMBIENT_LIGHT, lightState.m_bAmbientLight ? 1 : 0 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( jl_teeth_bump_ps30 );
				}
#endif
			}
			else
			{
...
#endif
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_vs20 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
					...
					SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( NUM_LIGHTS, bUseStaticControlFlow ? 0 : lightState.m_nNumLights );
					SET_DYNAMIC_VERTEX_SHADER( jl_teeth_vs20 );

					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_ps20b );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
						SET_DYNAMIC_PIXEL_SHADER( jl_teeth_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_ps20 );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER( jl_teeth_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_vs30 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( DYNAMIC_LIGHT, lightState.HasDynamicLight() );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( STATIC_LIGHT,  lightState.m_bStaticLight  ? 1 : 0 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING,  pShaderAPI->IsHWMorphingEnabled() );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
					SET_DYNAMIC_VERTEX_SHADER( jl_teeth_vs30 );

					DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_ps30 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( jl_teeth_ps30 );
				}
#endif
			}
	...
	void DrawFlashlight( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, VertexCompressionType_t vertexCompression )
	{
		SHADOW_STATE
		{
...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER( jl_teeth_flashlight_vs20 );
				SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
				SET_STATIC_VERTEX_SHADER( jl_teeth_flashlight_vs20 );

				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps20b );
					SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode );
					SET_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps20b );
				}
				else
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps20 );
					SET_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_STATIC_VERTEX_SHADER( jl_teeth_flashlight_vs30 );
				SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
				SET_STATIC_VERTEX_SHADER( jl_teeth_flashlight_vs30 );

				DECLARE_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps30 );
				SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode );
				SET_STATIC_PIXEL_SHADER( jl_teeth_flashlight_ps30 );
			}
#endif
			// On DX9, do sRGB
			pShaderShadow->EnableSRGBRead( SHADER_SAMPLER0, true );
		...
		DYNAMIC_STATE
		{
...
#endif
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_flashlight_vs20 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( jl_teeth_flashlight_vs20 );

				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
					SET_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_teeth_flashlight_vs30 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING,  pShaderAPI->IsHWMorphingEnabled() );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( jl_teeth_flashlight_vs30 );

				DECLARE_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
				SET_DYNAMIC_PIXEL_SHADER( jl_teeth_flashlight_ps30 );
			}
#endif
