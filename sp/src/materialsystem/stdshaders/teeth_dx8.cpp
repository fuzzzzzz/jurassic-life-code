...
#include "JL_teeth.inc"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

DEFINE_FALLBACK_SHADER( JL_Teeth, JL_Teeth_DX8 )

BEGIN_VS_SHADER( JL_Teeth_DX8, "Help for JL_Teeth_DX8" )
	...
	void DrawUsingVertexShader( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow )
	{
		SHADOW_STATE
		{
			...
			jl_teeth_Static_Index vshIndex;
			vshIndex.SetHALF_LAMBERT( IS_FLAG_SET( MATERIAL_VAR_HALFLAMBERT ) );
			vshIndex.SetINTRO( params[INTRO]->GetIntValue() != 0 );
			pShaderShadow->SetVertexShader( "JL_Teeth", vshIndex.GetIndex() );
			pShaderShadow->SetPixelShader( "JL_VertexLitTexture_Overbright2" );
		...
		DYNAMIC_STATE
		{
			...
			jl_teeth_Dynamic_Index vshIndex;
