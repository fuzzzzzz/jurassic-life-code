...
#include "JL_vertexlitgeneric_vs11.inc"
#include "JL_vertexlitgeneric_selfillumonly.inc"
...
DEFINE_FALLBACK_SHADER( JL_VertexLitGeneric, JL_VertexLitGeneric_DX8 )
DEFINE_FALLBACK_SHADER( JL_Skin_DX9, JL_VertexLitGeneric_DX8 )

BEGIN_VS_SHADER( JL_VertexLitGeneric_DX8, 
				"Help for JL_VertexLitGeneric" )
	BEGIN_SHADER_PARAMS
...
//		}
	...
	{	
		if ( IsPC() )
		{
			if ( g_pHardwareConfig->GetDXSupportLevel() < 70)
				return "JL_VertexLitGeneric_DX6";

			if ( g_pHardwareConfig->GetDXSupportLevel() < 80)
				return "JL_VertexLitGeneric_DX7";

			if ( g_pHardwareConfig->PreferReducedFillrate() )
				return "JL_VertexLitGeneric_NoBump_DX8";
		}
		return 0;
	}
	...
	inline const char *GetUnbumpedPixelShaderName( IMaterialVar** params, bool bSkipEnvmap )
	{
		...
		{
			"JL_VertexLitGeneric_EnvmapV2",
			"JL_VertexLitGeneric_SelfIlluminatedEnvmapV2",

			"JL_VertexLitGeneric_BaseAlphaMaskedEnvmapV2",
			"JL_VertexLitGeneric_SelfIlluminatedEnvmapV2",

			// Env map mask
			"JL_VertexLitGeneric_MaskedEnvmapV2",
			"JL_VertexLitGeneric_SelfIlluminatedMaskedEnvmapV2",

			"JL_VertexLitGeneric_MaskedEnvmapV2",
			"JL_VertexLitGeneric_SelfIlluminatedMaskedEnvmapV2",

			// Detail
			"JL_VertexLitGeneric_DetailEnvmapV2",
			"JL_VertexLitGeneric_DetailSelfIlluminatedEnvmapV2",

			"JL_VertexLitGeneric_DetailBaseAlphaMaskedEnvmapV2",
			"JL_VertexLitGeneric_DetailSelfIlluminatedEnvmapV2",

			// Env map mask
			"JL_VertexLitGeneric_DetailMaskedEnvmapV2",
			"JL_VertexLitGeneric_DetailSelfIlluminatedMaskedEnvmapV2",
		...
		if ( !params[BASETEXTURE]->IsTexture() )
		{
			if (params[ENVMAP]->IsTexture() && !bSkipEnvmap )
			{
				...
				{
					return "JL_VertexLitGeneric_EnvmapNoTexture";
				}
				else
				{
					return "JL_VertexLitGeneric_MaskedEnvmapNoTexture";
				}
			}
			else
			{
				...
				{
					return "JL_VertexLitGeneric_DetailNoTexture";
				}
				else
				{
					return "JL_VertexLitGeneric_NoTexture";
				}
			}
		}
		else
		{
			...
			{
				return "JL_VertexLitGeneric_BlendTint";
			}
			...
			else
			{
				...
				{
					if ( params[DETAIL]->IsTexture() )
						return "JL_VertexLitGeneric_DetailSelfIlluminated";
					else
						return "JL_VertexLitGeneric_SelfIlluminated";
				}
				else if ( params[DETAIL]->IsTexture() )
				{
					...
					{
						case 0:
							return "JL_VertexLitGeneric_Detail";

						case 1:								// additive modes
							return "JL_VertexLitGeneric_Detail_Additive";

						case 5:
						case 6:
							return "JL_VertexLitGeneric_Detail_Additive_selfillum";
							break;
							
						default:
							return "JL_VertexLitGeneric_Detail_LerpBase";
					}
				}
				else
				{
					return "JL_VertexLitGeneric";
				}
			}
		}
	}
	...
	void DrawUnbumpedUsingVertexShader( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, bool bSkipEnvmap )
	{
		SHADOW_STATE
		{
			...
			jl_vertexlitgeneric_vs11_Static_Index vshIndex;
			...
			pShaderShadow->SetVertexShader( "JL_vertexlitgeneric_vs11", vshIndex.GetIndex() );
...
//			if (params[ENVMAP]->IsTexture())
			...
			jl_vertexlitgeneric_vs11_Dynamic_Index vshIndex;
...
//-----------------------------------------------------------------------------
BEGIN_INHERITED_SHADER( JL_VertexLitGeneric_NoBump_DX8, JL_VertexLitGeneric_DX8,
			  "Help for JL_VertexLitGeneric_NoBump_DX8" )
