...
#include "JL_volume_clouds_vs20.inc"
#include "JL_volume_clouds_ps20.inc"
#include "JL_volume_clouds_ps20b.inc"
					  ...
					  IShaderShadow* pShaderShadow, VolumeCloudsVars_t &info, VertexCompressionType_t vertexCompression )
{
	SHADOW_STATE
	{
		...
		// Vertex Shader
		DECLARE_STATIC_VERTEX_SHADER( jl_volume_clouds_vs20 );
		SET_STATIC_VERTEX_SHADER( jl_volume_clouds_vs20 );
	
		// Pixel Shader
		if( g_pHardwareConfig->SupportsPixelShaders_2_b() && !IsOpenGL() ) // Always send POSIX down the 20 path (rg - why?)
		{
			DECLARE_STATIC_PIXEL_SHADER( jl_volume_clouds_ps20b );
			SET_STATIC_PIXEL_SHADER( jl_volume_clouds_ps20b );
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER( jl_volume_clouds_ps20 );
			SET_STATIC_PIXEL_SHADER( jl_volume_clouds_ps20 );
		}
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER( jl_volume_clouds_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
		SET_DYNAMIC_VERTEX_SHADER( jl_volume_clouds_vs20 );
		...
		if ( g_pHardwareConfig->SupportsPixelShaders_2_b() && !IsOpenGL() ) // Always send POSIX down the 20 path (rg - why?)
		{
			DECLARE_DYNAMIC_PIXEL_SHADER( jl_volume_clouds_ps20b );
			SET_DYNAMIC_PIXEL_SHADER( jl_volume_clouds_ps20b );
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER( jl_volume_clouds_ps20 );
			SET_DYNAMIC_PIXEL_SHADER( jl_volume_clouds_ps20 );
		}
