...
#include "JL_vr_distort_hud_ps20.inc"
#include "JL_vr_distort_hud_ps20b.inc"
#include "JL_vr_distort_hud_vs20.inc"
#include "JL_vr_distort_hud_ps30.inc"
#include "JL_vr_distort_hud_vs30.inc"
...
BEGIN_VS_SHADER( JL_vr_distort_hud, "Help for JL_hud warp" )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_VERTEX_SHADER( jl_vr_distort_hud_vs20 );
				SET_STATIC_VERTEX_SHADER( jl_vr_distort_hud_vs20 );

				if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps20b );
					SET_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps20b );
				}
				else
				{
					DECLARE_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps20 );
					SET_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps20 );
				}
			}
			else
			{
				DECLARE_STATIC_VERTEX_SHADER( jl_vr_distort_hud_vs30 );
				SET_STATIC_VERTEX_SHADER( jl_vr_distort_hud_vs30 );

				DECLARE_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps30 );
				SET_STATIC_PIXEL_SHADER( jl_vr_distort_hud_ps30 );
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			if ( !g_pHardwareConfig->SupportsShaderModel_3_0() )
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_vr_distort_hud_vs20 );
				SET_DYNAMIC_VERTEX_SHADER( jl_vr_distort_hud_vs20 );

				if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( CMBO_HUDUNDISTORT,	hudUndistortEnabled );
					SET_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( CMBO_HUDUNDISTORT,	hudUndistortEnabled );
					SET_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps20 );
				}
			}
			else
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( jl_vr_distort_hud_vs30 );
				SET_DYNAMIC_VERTEX_SHADER( jl_vr_distort_hud_vs30 );

				DECLARE_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( CMBO_HUDUNDISTORT,	hudUndistortEnabled );
				SET_DYNAMIC_PIXEL_SHADER( jl_vr_distort_hud_ps30 );
			}
