...
#include "JL_WorldVertexAlpha.inc"
#include "JL_worldvertexalpha_ps20.inc"
#include "JL_worldvertexalpha_ps20b.inc"

BEGIN_VS_SHADER( JL_WorldVertexAlpha, 
			  "Help for JL_WorldVertexAlpha" )
...
//		if( g_pHardwareConfig->GetDXSupportLevel() < 90 || g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE )
		{
			return "JL_WorldVertexAlpha_DX8";
		}
		return 0;
	}
		...
		if( g_pHardwareConfig->SupportsVertexAndPixelShaders() && !UsingEditor( params ) )
		{
			if( g_pHardwareConfig->GetDXSupportLevel() < 90 )
			{
				...
				SHADOW_STATE
				{
					...
					jl_worldvertexalpha_Static_Index vshIndex;
					pShaderShadow->SetVertexShader( "JL_WorldVertexAlpha", vshIndex.GetIndex() );

					pShaderShadow->SetPixelShader( "JL_WorldVertexAlpha" );
					FogToFogColor();
				}
				...
				DYNAMIC_STATE
				{
					...
					jl_worldvertexalpha_Dynamic_Index vshIndex;
			...
			else
			{
				...
				SHADOW_STATE
				{
					...
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_worldvertexalpha_ps20b );
						SET_STATIC_PIXEL_SHADER_COMBO( PASS, 0 );
						SET_STATIC_PIXEL_SHADER( jl_worldvertexalpha_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( jl_worldvertexalpha_ps20 );
						SET_STATIC_PIXEL_SHADER_COMBO( PASS, 0 );
						SET_STATIC_PIXEL_SHADER( jl_worldvertexalpha_ps20 );
					}
				...
				DYNAMIC_STATE
				{
					...
					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_worldvertexalpha_ps20b );
						SET_DYNAMIC_PIXEL_SHADER( jl_worldvertexalpha_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( jl_worldvertexalpha_ps20 );
						SET_DYNAMIC_PIXEL_SHADER( jl_worldvertexalpha_ps20 );
					}
				}
				Draw();
