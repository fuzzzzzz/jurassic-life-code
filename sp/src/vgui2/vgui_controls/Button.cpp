...
void Button::ApplySettings( KeyValues *inResourceData )
{
	...
	//JL
	SetScript(inResourceData->GetString("script", ""));
...
//[JL
const char *Button::GetScript()
{
	if (m_pScriptMessage != NULL)
	{
		return m_pScriptMessage->GetString("script");
	}
	return "";
}

void Button::SetScript(const char * str)
{
	if (m_pScriptMessage)
	{
		m_pScriptMessage->deleteThis();
	...
	m_pScriptMessage = new KeyValues("Script", "script", str);
}
//JL]